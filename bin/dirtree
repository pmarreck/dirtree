#!/usr/bin/env bash

# dirtree: A simple wrapper around eza for directory trees

DIRTREE_ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)

# Ensure mktemp outputs land on RAM-backed /tmp unless caller already set TMPDIR
export TMPDIR="${TMPDIR:-/tmp}"

show_help() {
	echo "dirtree - Display directory trees using eza"
	echo ""
	echo "Usage: dirtree [OPTIONS] [PATH]"
	echo ""
	echo "Options:"
	echo "  -h, --help         Show this help message"
	echo "  -a, --about        Show detailed description"
	echo "  -d, --depth N      Set maximum depth (default: 4)"
	echo "  --simple           Output a simple, LLM-friendly stateful tree"
	echo "  --decorated        Force decorated eza output (even when piped)"
	echo "  --no-icons         Disable icons (simple mode + decorated header)"
	echo "  --no-color        Disable ANSI colors and persist preference"
	echo "  --no-hyperlinks   Disable OSC8 hyperlinks and persist preference"
	echo "  --default X        Persist default state: opened|closed"
	echo "  -o, --open DIR...  Open one or more subdirs (repeat flag to add more)"
	echo "  -c, --close DIR... Close one or more subdirs (repeat flag to add more)"
	echo "  --show PATH...     Force show relative paths; wrap regexes as /pattern/ or !/pattern/"
	echo "  --hide PATH...     Hide relative paths; wrap regexes as /pattern/ or !/pattern/ (repeatable)"
	echo "  --sort MODE        Sorting mode: modified|alpha (default: modified)"
	echo "  --asc              Sort ascending"
	echo "  --desc             Sort descending (default)"
	echo "  --show-hidden      Temporarily display paths hidden via config"
	echo "  --rewrite-settings Rewrite state file using current settings"
	echo "  --test             Run associated tests"
	echo ""
	echo "Use /pattern/ or !/pattern/ with --open/--close/--show/--hide to add regex rules; other arguments are treated as literals."
	echo "Paths supplied to --show/--hide must be relative (no leading '/')."
	echo ""
	echo "Behavior: By default, when stdout is not a TTY (piped),"
	echo "  colors/icons/hyperlinks are disabled unless --decorated is given."
	echo ""
	echo "Examples:"
	echo "  dirtree                       # Show tree of current directory"
	echo "  dirtree -d 3                  # Set depth to 3 levels"
	echo "  dirtree --sort alpha --asc    # Sorted alphabetically ascending"
}

# Default values
depth=4 # default depth aligns with docs/tests
dir="."
simple_mode=false
force_decorated=false
user_disabled_icons=false
cli_color_override=false
cli_color_value=""
cli_hyperlink_override=false
cli_hyperlink_value=""
color_pref_override=""
hyperlink_pref_override=""
default_state=""
default_state_set=false
default_visibility="shown"
default_visibility_set=false
default_conflict=""
cli_default_state=""
cli_default_visibility=""
cli_depth_override=false
cli_depth_value=""
cli_sort_mode_override=false
cli_sort_mode_value=""
cli_sort_direction_override=false
cli_sort_direction_value=""
# smarts: reveal SCM changes unless explicitly disabled
priority_enabled=false
declare -A priority_files
declare -A priority_dirs
open_args=()
close_args=()
show_args=()
hide_args=()
open_regex_args=()
close_regex_args=()
show_regex_args=()
hide_regex_args=()
state_modified=false
sort_mode="modified"
sort_reverse=1  # 1 => descending (default), 0 => ascending
render_sort_field="modified"
render_sort_reverse=1
dir_pending=true
show_hidden=false
rewrite_settings=false

STATE_VERSION_LABEL="ver=1.1"
STATE_HEADER_COMMENT="# Dirtree: Stateful directory trees in the CLI for humans and LLMs. https://github.com/pmarreck/dirtree"
state_version=""
state_needs_migration=false
declare -a state_passthrough_lines=()
persisted_depth=""
persisted_depth_set=false
persisted_sort_mode=""
persisted_sort_mode_set=false
persisted_sort_direction=""
persisted_sort_direction_set=false
persisted_color_preference=""
persisted_color_preference_set=false
persisted_hyperlink_preference=""
persisted_hyperlink_preference_set=false
inherited_default_state=""
inherited_default_state_set=false
inherited_default_visibility="shown"
inherited_default_visibility_set=false
inherited_depth=""
inherited_depth_set=false
inherited_sort_mode=""
inherited_sort_mode_set=false
inherited_sort_direction=""
inherited_sort_direction_set=false
inherited_color_preference=""
inherited_color_preference_set=false
inherited_hyperlink_preference=""
inherited_hyperlink_preference_set=false
declare -A inherited_open_literals
declare -A inherited_close_literals
declare -A inherited_show_literals
declare -A inherited_hide_literals
declare -A inherited_open_regex_map
declare -A inherited_close_regex_map
declare -A inherited_show_regex_map
declare -A inherited_hide_regex_map
declare -A effective_open_literals
declare -A effective_close_literals
declare -A effective_show_literals
declare -A effective_hide_literals
declare -a effective_open_regexes
declare -a effective_close_regexes
declare -a effective_show_regexes
declare -a effective_hide_regexes
effective_default_state=""
effective_default_visibility="shown"
effective_depth=""
effective_sort_mode=""
effective_sort_direction=""
effective_color_preference=""
effective_hyperlink_preference=""
declare -a open_regex_tests
declare -a close_regex_tests
declare -a show_regex_tests
declare -a hide_regex_tests
declare -A dir_children_cache
REGEX_SPEC_SEP=$'\x1f'
variadic_arg_count=0

stdout_is_tty() {
	if [[ -n "${PIPED_STDOUT:-}" ]]; then
		local flag="${PIPED_STDOUT,,}"
		case "$flag" in
			0|false|no|off)
				return 0
				;;
			1|true|yes|on)
				return 1
				;;
		esac
	fi
	[[ -t 1 ]]
}

# Normalize path to a relative, trimmed form
normalize_rel() {
	local base="$1" p="$2"
	# remove leading ./
	p="${p#./}"
	# strip leading base if absolute within base
	case "$p" in
		"$base"/*) p="${p#"$base"/}" ;;
	esac
	# strip leading slash if any
	p="${p#/}"
	# strip trailing slash
	p="${p%/}"
	printf '%s' "$p"
}

osc8_decode_path() {
	local path="$1" out="" char hex
	local i len=${#path}
	for ((i=0; i<len; i++)); do
		char=${path:i:1}
		if [[ "$char" == '%' && i+2 < len ]]; then
			hex=${path:i+1:2}
			if [[ "$hex" =~ ^[0-9A-Fa-f]{2}$ ]]; then
				printf -v char '\\x%s' "$hex"
				out+=$(printf '%b' "$char")
				((i+=2))
				continue
			fi
		fi
		out+="${path:i:1}"
	done
	printf '%s' "$out"
}

declare -A open_literals
declare -A close_literals
declare -A show_literals
declare -A hide_literals
declare -A open_literal_comments
declare -A close_literal_comments
declare -A show_literal_comments
declare -A hide_literal_comments
declare -A open_regex_comments
declare -A close_regex_comments
declare -A show_regex_comments
declare -A hide_regex_comments
open_regexes=()
close_regexes=()
show_regexes=()
hide_regexes=()

hidden_count_log=""

note_hidden() {
	local kind="$1"
	local rel="$2"
	if [[ -n "$rel" ]]; then
		case "$rel" in
			*.dirtree-state|.dirtree-state)
				return
				;;
		esac
	fi
	[[ "$show_hidden" == true ]] && return
	[[ -z "$hidden_count_log" ]] && return
	printf '%s\n' "$kind" >>"$hidden_count_log"
}

is_glob_pattern() {
	local token="$1"
	case "$token" in
		re:*|RE:*) return 1 ;;
	esac
	case "$token" in
		*\\*) return 1 ;;
	esac
	case "$token" in
		*\**|*\?*|*\[*)
			return 0
			;;
	esac
	return 1
}

glob_to_regex() {
	local glob="$1"
	local regex="^"
	local len=${#glob}
	local i=0
	while (( i < len )); do
		local c=${glob:i:1}
		case "$c" in
			'*')
				if (( i+1 < len )) && [[ ${glob:i+1:1} == "*" ]]; then
					if (( i+2 < len )) && [[ ${glob:i+2:1} == "/" ]]; then
						regex+="(.*/)?"
						((i+=3))
					else
						regex+=".*"
						((i+=2))
					fi
					continue
				else
					regex+="[^/]*"
				fi
				;;
			'?')
				regex+="[^/]"
				;;
			'[')
				local j=$((i+1))
				local class="["
				if (( j < len )); then
					local first=${glob:j:1}
					if [[ "$first" == "!" || "$first" == "^" ]]; then
						class+="^"
						((j++))
					fi
				fi
				if (( j < len )) && [[ ${glob:j:1} == ']' ]]; then
					class+="]"
					((j++))
				fi
				local valid=0
				while (( j < len )); do
					local ch=${glob:j:1}
					if [[ "$ch" == "]" ]]; then
						valid=1
						((j++))
						break
					fi
					case "$ch" in
						'\'|'-'|'['|']'|'^')
							class+="\\$ch"
							;;
						*)
							class+="$ch"
							;;
					esac
					((j++))
				done
				if (( valid == 0 )); then
					regex+="\\["
					((i++))
					continue
				fi
				class+="]"
				regex+="$class"
				i=$j
				continue
				;;
			*)
				case "$c" in
					'.'|'+'|'?'|'^'|'$'|'('|')'|'{'|'}'|'|'|'\\')
						regex+="\\$c"
						;;
					*)
						regex+="$c"
						;;
				esac
				;;
		esac
		((i++))
		continue
	done
	regex+="$"
	printf '%s' "$regex"
}

collect_variadic_args() {
	local -n _target=$1
	local allow_raw="$2"
	shift 2
	local allow_list=""
	if [[ -n "$allow_raw" ]]; then
		allow_list=" $allow_raw "
	fi
	_target=()
	variadic_arg_count=0
	while [[ $# -gt 0 ]]; do
		local token="$1"
		if [[ "$token" == --* || "$token" == -* ]]; then
			break
		fi
		if [[ "$dir_pending" == true && $# -eq 1 && ${#_target[@]} -gt 0 ]]; then
			if [[ -z "$allow_list" ]]; then
				break
			fi
			local token_lc="${token,,}"
			if [[ "$allow_list" != *" $token_lc "* ]]; then
				break
			fi
		fi
		_target+=("$token")
		shift
		((variadic_arg_count++))
	done
}

build_regex_tests() {
	local -n _source=$1
	local -n _dest=$2
	_dest=()
	local token pattern neg
	for token in "${_source[@]}"; do
		[[ -z "$token" ]] && continue
		pattern="$token"
		neg=0
		if [[ "$pattern" == \!* ]]; then
			neg=1
			pattern="${pattern:1}"
		fi
		_dest+=("$neg$REGEX_SPEC_SEP$pattern")
	done
}

clear_priority_maps() {
	local key
	for key in "${!priority_files[@]}"; do
		unset 'priority_files[$key]'
	done
	for key in "${!priority_dirs[@]}"; do
		unset 'priority_dirs[$key]'
	done
}

add_priority_parents() {
	local p="$1"
	while [[ "$p" == */* ]]; do
		p="${p%/*}"
		[[ -z "$p" ]] && break
		priority_dirs["$p"]=1
	done
}

add_priority_path() {
	local repo_root="$1"
	local rel_path="$2"
	[[ -z "$rel_path" ]] && return
	local abs_path="$repo_root/$rel_path"
	if [[ ! -e "$abs_path" && ! -L "$abs_path" ]]; then
		return
	fi
	local relative
	relative=$(realpath --relative-to="$abs_dir" "$abs_path" 2>/dev/null) || relative=""
	if [[ -z "$relative" || "$relative" == ..* ]]; then
		relative="$rel_path"
	fi
	if [[ -d "$abs_path" ]]; then
		priority_dirs["$relative"]=1
		add_priority_parents "$relative"
		while IFS= read -r -d '' child; do
			local rel_child
			rel_child=$(realpath --relative-to="$abs_dir" "$child" 2>/dev/null) || continue
			if [[ "$rel_child" == ..* ]]; then
				continue
			fi
			priority_files["$rel_child"]=1
			add_priority_parents "$rel_child"
		done < <(find "$abs_path" \( -type f -o -type l \) -print0)
	else
		priority_files["$relative"]=1
		add_priority_parents "$relative"
	fi
}

collect_priority_paths() {
	local base="$1"
	priority_enabled=false
	clear_priority_maps
	if command -v git >/dev/null 2>&1; then
		if git -C "$base" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
			local repo_root
			repo_root=$(git -C "$base" rev-parse --show-toplevel 2>/dev/null) || repo_root=""
			if [[ -n "$repo_root" ]]; then
				repo_root=$(cd "$repo_root" && pwd -P)
			fi
			if [[ -n "$repo_root" ]]; then
				while IFS= read -r -d '' record; do
					[[ -z "$record" ]] && continue
					local path="${record:3}"
					if [[ -z "$path" ]]; then
						continue
					fi
					if [[ "$path" == *" -> "* ]]; then
						path="${path##* -> }"
					fi
					add_priority_path "$repo_root" "$path"
					# Treat deleted files (status starting with ' D' etc.) even if absent by checking parent directories
					if [[ -e "$repo_root/$path" || -L "$repo_root/$path" ]]; then
						continue
					fi
					local rel_dir
					rel_dir=$(dirname -- "$path")
					if [[ "$rel_dir" != "." ]]; then
						local abs_dir_local="$repo_root/$rel_dir"
						if [[ -d "$abs_dir_local" ]]; then
							local rel_to_target
							rel_to_target=$(realpath --relative-to="$abs_dir" "$abs_dir_local" 2>/dev/null) || rel_to_target=""
							if [[ -n "$rel_to_target" && "$rel_to_target" != ..* ]]; then
								priority_dirs["$rel_to_target"]=1
								add_priority_parents "$rel_to_target"
							fi
						fi
					fi
				done < <(git -C "$repo_root" status --porcelain -z)
			fi
		fi
	fi
	if [[ ${#priority_files[@]} -gt 0 || ${#priority_dirs[@]} -gt 0 ]]; then
		priority_enabled=true
		return
	fi
	if command -v jj >/dev/null 2>&1; then
		local repo_root=""
		if [[ -d "$base/.jj" && ! -f "$base/.jj/repo/store/type" ]]; then
			repo_root=""
		else
			repo_root=$(jj -R "$base" root 2>/dev/null) || repo_root=""
		fi
		if [[ -n "$repo_root" ]]; then
			while IFS= read -r path; do
				[[ -z "$path" ]] && continue
				add_priority_path "$repo_root" "$path"
			done < <(jj -R "$base" diff --name-only)
		fi
	fi
	if [[ ${#priority_files[@]} -gt 0 || ${#priority_dirs[@]} -gt 0 ]]; then
		priority_enabled=true
	fi
}

process_default_token() {
	local token="$1"
	[[ -z "$token" ]] && return 0
	if [[ -n "$default_conflict" ]]; then
		return 1
	fi
	token=$(printf '%s' "$token" | tr '[:upper:]' '[:lower:]')
	case "$token" in
		open|opened)
			if [[ "$default_state_set" == true && "$default_state" != "opened" ]]; then
				default_conflict="default entries conflict: opened and closed"
				return 1
			fi
			[[ "$token" == "open" ]] && state_needs_migration=true
			default_state="opened"
			default_state_set=true
			;;
		close|closed)
			if [[ "$default_state_set" == true && "$default_state" != "closed" ]]; then
				default_conflict="default entries conflict: opened and closed"
				return 1
			fi
			[[ "$token" == "close" ]] && state_needs_migration=true
			default_state="closed"
			default_state_set=true
			;;
		show|shown)
			if [[ "$default_visibility_set" == true && "$default_visibility" != "shown" ]]; then
				default_conflict="default entries conflict: shown and hidden"
				return 1
			fi
			[[ "$token" == "show" ]] && state_needs_migration=true
			default_visibility="shown"
			default_visibility_set=true
			;;
		hide|hidden)
			if [[ "$default_visibility_set" == true && "$default_visibility" != "hidden" ]]; then
				default_conflict="default entries conflict: shown and hidden"
				return 1
			fi
			[[ "$token" == "hide" ]] && state_needs_migration=true
			default_visibility="hidden"
			default_visibility_set=true
			;;
		*)
			default_conflict="default entries conflict: unknown entry '$token'"
			return 1
			;;
	esac
	return 0
}

report_hidden_counts() {
	[[ "$show_hidden" == true ]] && return
	[[ -z "$hidden_count_log" ]] && return
	[[ ! -f "$hidden_count_log" ]] && return
	local dir_count=0 file_count=0
	if [[ -s "$hidden_count_log" ]]; then
		dir_count=$(grep -c '^dir$' "$hidden_count_log" || true)
		file_count=$(grep -c '^file$' "$hidden_count_log" || true)
	fi
	if (( dir_count == 0 && file_count == 0 )); then
		return
	fi
	local parts=()
	local msg=""
	if (( dir_count > 0 )); then
		if (( dir_count == 1 )); then
			parts+=("1 directory")
		else
			parts+=("${dir_count} directories")
		fi
	fi
	if (( file_count > 0 )); then
		if (( file_count == 1 )); then
			parts+=("1 file")
		else
			parts+=("${file_count} files")
		fi
	fi
	if (( ${#parts[@]} == 2 )); then
		msg="${parts[0]} and ${parts[1]}"
	else
		msg="${parts[0]}"
	fi
	local total=$((dir_count + file_count))
	if (( total == 1 )); then
		msg+=" is hidden."
	else
		msg+=" are hidden."
	fi
	if [[ "$simple_mode" == true ]]; then
		printf '\n%s\n' "$msg" >&2
	else
		printf '\n\033[2;3m%s\033[0m\n' "$msg" >&2
	fi
}

open_match_type="none"
open_match_pattern=""
close_match_type="none"
close_match_pattern=""
show_match_type="none"
show_match_pattern=""
hide_match_type="none"
hide_match_pattern=""

append_unique_regex() {
	local -n array_ref="$1"
	local pattern="$2"
	[[ -z "$pattern" ]] && return
	for existing in "${array_ref[@]}"; do
		if [[ "$existing" == "$pattern" ]]; then
			return
		fi
	done
	array_ref+=("$pattern")
}

parse_wrapped_regex_token() {
	local token="$1"
	local result_var="$2"
	local neg=false
	if [[ "$token" == \!/* && "$token" == */ ]]; then
		neg=true
		if (( ${#token} <= 3 )); then
			return 2
		fi
		local inner="${token:2:-1}"
		[[ -z "$inner" ]] && return 2
		if [[ "$neg" == true ]]; then
			printf -v "$result_var" '!%s' "$inner"
		else
			printf -v "$result_var" '%s' "$inner"
		fi
		return 0
	elif [[ "$token" == /* && "$token" == */ ]]; then
		if (( ${#token} <= 2 )); then
			return 2
		fi
		local inner="${token:1:-1}"
		[[ -z "$inner" ]] && return 2
		printf -v "$result_var" '%s' "$inner"
		return 0
	fi
	return 1
}

wrap_regex_pattern() {
	local token="$1"
	if [[ "$token" == \!* ]]; then
		printf '!/%s/' "${token:1}"
	else
		printf '/%s/' "$token"
	fi
}

regex_token_matches() {
	local token="$1"
	local rel="$2"
	local pattern="$token"
	local neg=false
	if [[ "$pattern" == \!* ]]; then
		neg=true
		pattern="${pattern:1}"
	fi
	if [[ -z "$pattern" ]]; then
		return 1
	fi
	if [[ "$neg" == true ]]; then
		if [[ "$rel" =~ $pattern ]]; then
			return 1
		else
			return 0
		fi
	else
		if [[ "$rel" =~ $pattern ]]; then
			return 0
		else
			return 1
		fi
	fi
}

is_wrapped_regex_string() {
	local token="$1"
	[[ ${token: -1} == "/" ]] || return 1
	case "$token" in
		/*) return 0 ;;
		!/*) return 0 ;;
		*) return 1 ;;
	esac
}

load_literal_list() {
	local base="$1" list="$2" literal_name="$3" regex_name="$4"
	local -n literals_ref="$literal_name"
	local -n regexes_ref="$regex_name"
	IFS=';' read -r -a arr <<< "$list"
	for entry in "${arr[@]}"; do
		[[ -z "$entry" ]] && continue
		local norm
		norm=$(normalize_rel "$base" "$entry")
		[[ -n "$norm" ]] && literals_ref["$norm"]=1
	done
}

load_regex_list() {
	local list="$1" regex_name="$2"
	local -n regexes_ref="$regex_name"
	IFS=';' read -r -a arr <<< "$list"
	for entry in "${arr[@]}"; do
		[[ -z "$entry" ]] && continue
		append_unique_regex regexes_ref "$entry"
	done
}

load_hide_list() {
	local base="$1" list="$2" pattern
	IFS=';' read -r -a arr <<< "$list"
	for entry in "${arr[@]}"; do
		[[ -z "$entry" ]] && continue
		local pattern=""
		parse_wrapped_regex_token "$entry" pattern
		case $? in
			0)
				append_unique_regex hide_regexes "$pattern"
				continue
				;;
			2)
				continue
				;;
		esac
		local norm
		norm=$(normalize_rel "$base" "$entry")
		[[ -n "$norm" ]] && hide_literals["$norm"]=1
	done
}

load_show_list() {
	local base="$1" list="$2" pattern
	IFS=';' read -r -a arr <<< "$list"
	for entry in "${arr[@]}"; do
		[[ -z "$entry" ]] && continue
		pattern=""
		parse_wrapped_regex_token "$entry" pattern
		case $? in
			0)
				append_unique_regex show_regexes "$pattern"
				continue
				;;
			2)
				continue
				;;
		esac
		local norm
		norm=$(normalize_rel "$base" "$entry")
		[[ -n "$norm" ]] && show_literals["$norm"]=1
	done
}

strip_leading_space_tabs() {
	local value="$1"
	if [[ "$value" =~ ^[[:space:]]*(.*)$ ]]; then
		printf '%s' "${BASH_REMATCH[1]}"
	else
		printf '%s' "$value"
	fi
}

strip_trailing_space_tabs() {
	local value="$1"
	while [[ -n "$value" ]]; do
		local last="${value: -1}"
		if [[ "$last" == ' ' || "$last" == $'\t' ]]; then
			value="${value::-1}"
		else
			break
		fi
	done
	printf '%s' "$value"
}

is_state_header_comment() {
	local line="$1"
	line=$(strip_leading_space_tabs "$line")
	[[ "$line" == "$STATE_HEADER_COMMENT" ]]
}

split_inline_items() {
	local content="$1"
	local ref_name="$2"
	local -n arr_ref="$ref_name"
	arr_ref=()
	content=$(strip_leading_space_tabs "$content")
	content=$(strip_trailing_space_tabs "$content")
	[[ -z "$content" ]] && return
	local sentinel=$'\x1f'
	content="${content//\\ /$sentinel}"
	local -a parts=()
	read -r -a parts <<< "$content"
	for part in "${parts[@]}"; do
		arr_ref+=("${part//$sentinel/ }")
	done
}

parse_inline_default() {
	local content="$1"
	local -a tokens=()
	split_inline_items "$content" tokens
	for token in "${tokens[@]}"; do
		process_default_token "$token"
	done
}

parse_inline_collection() {
	local base="$1" key="$2" content="$3"
	local -a tokens=()
	split_inline_items "$content" tokens
	local token pattern norm
	for token in "${tokens[@]}"; do
		[[ -z "$token" ]] && continue
		pattern=""
		parse_wrapped_regex_token "$token" pattern
		case $? in
			0)
				case "$key" in
					open) append_unique_regex open_regexes "$pattern" ;;
					close) append_unique_regex close_regexes "$pattern" ;;
					show) append_unique_regex show_regexes "$pattern" ;;
					hide) append_unique_regex hide_regexes "$pattern" ;;
				esac
				continue
				;;
			2)
				continue
				;;
		esac
		norm=$(normalize_rel "$base" "$token")
		[[ -z "$norm" ]] && continue
		case "$key" in
			open) open_literals["$norm"]=1 ;;
			close) close_literals["$norm"]=1 ;;
			show) show_literals["$norm"]=1 ;;
			hide) hide_literals["$norm"]=1 ;;
		esac
	done
}

read_state() {
	local base="$1"
	local state_file="$base/.dirtree-state"
	default_state=""
	default_state_set=false
	default_visibility="shown"
	default_visibility_set=false
	default_conflict=""
	open_literals=()
	close_literals=()
	show_literals=()
	hide_literals=()
	open_regexes=()
	close_regexes=()
	show_regexes=()
	hide_regexes=()
	state_version="ver=1.0"
	state_needs_migration=false
	state_passthrough_lines=()
	persisted_depth=""
	persisted_depth_set=false
	persisted_sort_mode=""
	persisted_sort_mode_set=false
	persisted_sort_direction=""
	persisted_sort_direction_set=false
	persisted_color_preference=""
	persisted_color_preference_set=false
	persisted_hyperlink_preference=""
	persisted_hyperlink_preference_set=false
	[[ ! -f "$state_file" ]] && return

	local -a __state_lines=()
	mapfile -t __state_lines < "$state_file"
	local format="undetermined"
	local idx=0
	local count=${#__state_lines[@]}
	local current_array=""
	local collecting_unknown=0
	local -a unknown_buffer=()
	local array_comment_buffer=""

	while (( idx < count )); do
		local raw="${__state_lines[$idx]}"
		local line="${raw%$'\r'}"

		if [[ -n "$current_array" ]]; then
			if (( collecting_unknown )); then
				unknown_buffer+=("$raw")
				if [[ "${line//[[:space:]]/}" == "]" ]]; then
					state_passthrough_lines+=("${unknown_buffer[@]}")
					unknown_buffer=()
					collecting_unknown=0
					current_array=""
				fi
				((idx++))
				continue
			fi

			if [[ "${line//[[:space:]]/}" == "]" ]]; then
				current_array=""
				array_comment_buffer=""
				((idx++))
				continue
			fi

			if [[ "$line" =~ ^[[:space:]]*# ]]; then
				case "$current_array" in
					default|open|close|show|hide)
						array_comment_buffer+="$raw\n"
						;;
					*) ;;
				esac
				((idx++))
				continue
			fi

			local entry
			entry=$(strip_leading_space_tabs "$line")
			entry=$(strip_trailing_space_tabs "$entry")
			[[ -z "$entry" ]] && { ((idx++)); continue; }

		pattern=""
		parse_wrapped_regex_token "$entry" pattern
		case $? in
			0)
				case "$current_array" in
					open)
						append_unique_regex open_regexes "$pattern"
						if [[ -n "$array_comment_buffer" ]]; then
							local key
							key=$(wrap_regex_pattern "$pattern")
							open_regex_comments["$key"]="$array_comment_buffer"
						fi
						;;
					close)
						append_unique_regex close_regexes "$pattern"
						if [[ -n "$array_comment_buffer" ]]; then
							local key
							key=$(wrap_regex_pattern "$pattern")
							close_regex_comments["$key"]="$array_comment_buffer"
						fi
						;;
					show)
						append_unique_regex show_regexes "$pattern"
						if [[ -n "$array_comment_buffer" ]]; then
							local key
							key=$(wrap_regex_pattern "$pattern")
							show_regex_comments["$key"]="$array_comment_buffer"
						fi
						;;
					hide)
						append_unique_regex hide_regexes "$pattern"
						if [[ -n "$array_comment_buffer" ]]; then
							local key
							key=$(wrap_regex_pattern "$pattern")
							hide_regex_comments["$key"]="$array_comment_buffer"
						fi
						;;
				esac
				array_comment_buffer=""
				((idx++))
				continue
				;;
			2)
				array_comment_buffer=""
				((idx++))
				continue
				;;
		esac

			local norm
			norm=$(normalize_rel "$base" "$entry")
			[[ -z "$norm" ]] && { ((idx++)); continue; }
				case "$current_array" in
					open)
						open_literals["$norm"]=1
						[[ -n "$array_comment_buffer" ]] && open_literal_comments["$norm"]="$array_comment_buffer"
						;;
					close)
						close_literals["$norm"]=1
						[[ -n "$array_comment_buffer" ]] && close_literal_comments["$norm"]="$array_comment_buffer"
						;;
					show)
						show_literals["$norm"]=1
						[[ -n "$array_comment_buffer" ]] && show_literal_comments["$norm"]="$array_comment_buffer"
						;;
					hide)
						hide_literals["$norm"]=1
						[[ -n "$array_comment_buffer" ]] && hide_literal_comments["$norm"]="$array_comment_buffer"
						;;
				esac
			array_comment_buffer=""

			((idx++))
			continue
		fi

		if [[ "$format" == "undetermined" ]]; then
			local no_ws="${line//[[:space:]]/}"
			if [[ -z "$no_ws" ]]; then
				((idx++))
				continue
			fi
			if [[ "$line" =~ ^[[:space:]]*# ]]; then
				if ! is_state_header_comment "$line"; then
					state_passthrough_lines+=("$raw")
				fi
				((idx++))
				continue
			fi
			if [[ "$line" =~ ^ver[[:space:]]*=[[:space:]]*([0-9.]+)$ ]]; then
				state_version="ver=${BASH_REMATCH[1]}"
				format="inima"
				((idx++))
				continue
			fi
			if [[ "$line" =~ ^ver([0-9.]+)$ ]]; then
				state_version="ver=${BASH_REMATCH[1]}"
				format="inima"
				state_needs_migration=true
				((idx++))
				continue
			fi
			format="legacy"
		fi

		if [[ "$format" == "legacy" ]]; then
		if [[ "$line" =~ ^[[:space:]]*# ]]; then
			if ! is_state_header_comment "$line"; then
				state_passthrough_lines+=("$raw")
			fi
			((idx++))
			continue
		fi
		case "$line" in
			default=*)
				local legacy_value="${line#default=}"
				legacy_value=${legacy_value//;/ }
				for token in $legacy_value; do
					process_default_token "$token"
				 done
				;;
				open_regex=*) load_regex_list "${line#open_regex=}" open_regexes ;;
				close_regex=*) load_regex_list "${line#close_regex=}" close_regexes ;;
				show_regex=*) load_regex_list "${line#show_regex=}" show_regexes ;;
				hide_regex=*) load_regex_list "${line#hide_regex=}" hide_regexes ;;
				open=*) load_literal_list "$base" "${line#open=}" open_literals open_regexes ;;
				close=*) load_literal_list "$base" "${line#close=}" close_literals close_regexes ;;
				show=*) load_show_list "$base" "${line#show=}" ;;
				hide=*) load_hide_list "$base" "${line#hide=}" ;;
				*) state_passthrough_lines+=("$raw") ;;
			esac
			((idx++))
			continue
		fi

		if [[ "$line" =~ ^[[:space:]]*# ]]; then
			if ! is_state_header_comment "$line"; then
				state_passthrough_lines+=("$raw")
			fi
			((idx++))
			continue
		fi
		if [[ -z "${line//[[:space:]]/}" ]]; then
			((idx++))
			continue
		fi

		if [[ "$line" =~ ^([A-Za-z0-9_]+)[[:space:]]*=(.*)$ ]]; then
			local key="${BASH_REMATCH[1]}"
			local value="${BASH_REMATCH[2]}"
			if [[ ${value:0:1} == " " ]]; then
				value="${value:1}"
			fi
			local compact="${value//[[:space:]]/}"
			case "$key" in
				default)
					value=$(strip_trailing_space_tabs "$(strip_leading_space_tabs "$value")")
					if [[ "$compact" == "[" ]]; then
						current_array="default"
						collecting_unknown=0
						array_comment_buffer=""
					elif [[ "$value" =~ ^\[[^][]*\]$ ]]; then
						local inner="${value:1:-1}"
						parse_inline_default "$inner"
						state_needs_migration=true
					else
						value=${value//;/ }
						for token in $value; do
							process_default_token "$token"
						done
					fi
					;;
				depth)
					value=$(strip_trailing_space_tabs "$(strip_leading_space_tabs "$value")")
					if [[ "$value" =~ ^[0-9]+$ ]]; then
						persisted_depth="$value"
						persisted_depth_set=true
					else
						state_passthrough_lines+=("$raw")
					fi
					;;
				sort)
					value=$(strip_trailing_space_tabs "$(strip_leading_space_tabs "$value")")
					case "$value" in
						alpha|modified)
							persisted_sort_mode="$value"
							persisted_sort_mode_set=true
							;;
						*)
							state_passthrough_lines+=("$raw")
							;;
					esac
					;;
				sort_direction)
					value=$(strip_trailing_space_tabs "$(strip_leading_space_tabs "$value")")
					case "$value" in
						asc|desc)
							persisted_sort_direction="$value"
							persisted_sort_direction_set=true
							;;
						*)
							state_passthrough_lines+=("$raw")
							;;
					esac
					;;
				color)
					value=$(strip_trailing_space_tabs "$(strip_leading_space_tabs "$value")")
					case "$value" in
						true|false)
							persisted_color_preference="$value"
							persisted_color_preference_set=true
							;;
						*)
							state_passthrough_lines+=("$raw")
							;;
					esac
					;;
				hyperlink)
					value=$(strip_trailing_space_tabs "$(strip_leading_space_tabs "$value")")
					case "$value" in
						true|false)
							persisted_hyperlink_preference="$value"
							persisted_hyperlink_preference_set=true
							;;
						*)
							state_passthrough_lines+=("$raw")
							;;
					esac
					;;
				open|close|show|hide)
					value=$(strip_trailing_space_tabs "$(strip_leading_space_tabs "$value")")
					if [[ "$compact" == "[" ]]; then
						current_array="$key"
						collecting_unknown=0
						array_comment_buffer=""
					elif [[ "$value" =~ ^\[[^][]*\]$ ]]; then
						local inner="${value:1:-1}"
						parse_inline_collection "$base" "$key" "$inner"
						state_needs_migration=true
					else
						state_passthrough_lines+=("$raw")
					fi
					;;
				*)
					if [[ "$compact" == "[" ]]; then
						current_array="$key"
						collecting_unknown=1
						unknown_buffer=("$raw")
					else
						state_passthrough_lines+=("$raw")
					fi
					;;
			esac
		else
			state_passthrough_lines+=("$raw")
		fi
		((idx++))
	done

	if [[ -n "$default_conflict" ]]; then
		echo "Error: $default_conflict" >&2
		exit 1
	fi

	if [[ "$format" == "legacy" ]]; then
		state_needs_migration=true
	else
		[[ "$state_version" != "$STATE_VERSION_LABEL" ]] && state_needs_migration=true
	fi
}

reset_inherited_state() {
	inherited_default_state=""
	inherited_default_state_set=false
	inherited_default_visibility="shown"
	inherited_default_visibility_set=false
	inherited_depth=""
	inherited_depth_set=false
	inherited_sort_mode=""
	inherited_sort_mode_set=false
	inherited_sort_direction=""
	inherited_sort_direction_set=false
	inherited_color_preference=""
	inherited_color_preference_set=false
	inherited_hyperlink_preference=""
	inherited_hyperlink_preference_set=false
	inherited_open_literals=()
	inherited_close_literals=()
	inherited_show_literals=()
	inherited_hide_literals=()
	inherited_open_regex_map=()
	inherited_close_regex_map=()
	inherited_show_regex_map=()
	inherited_hide_regex_map=()
}

merge_inherited_from_current() {
	if [[ "$default_state_set" == true && -n "$default_state" ]]; then
		inherited_default_state="$default_state"
		inherited_default_state_set=true
	fi
	if [[ "$default_visibility_set" == true && -n "$default_visibility" ]]; then
		inherited_default_visibility="$default_visibility"
		inherited_default_visibility_set=true
	fi
	if [[ "$persisted_depth_set" == true && -n "$persisted_depth" ]]; then
		inherited_depth="$persisted_depth"
		inherited_depth_set=true
	fi
	if [[ "$persisted_sort_mode_set" == true && -n "$persisted_sort_mode" ]]; then
		inherited_sort_mode="$persisted_sort_mode"
		inherited_sort_mode_set=true
	fi
	if [[ "$persisted_sort_direction_set" == true && -n "$persisted_sort_direction" ]]; then
		inherited_sort_direction="$persisted_sort_direction"
		inherited_sort_direction_set=true
	fi
	if [[ "$persisted_color_preference_set" == true && -n "$persisted_color_preference" ]]; then
		inherited_color_preference="$persisted_color_preference"
		inherited_color_preference_set=true
	fi
	if [[ "$persisted_hyperlink_preference_set" == true && -n "$persisted_hyperlink_preference" ]]; then
		inherited_hyperlink_preference="$persisted_hyperlink_preference"
		inherited_hyperlink_preference_set=true
	fi

	local key
	for key in "${!open_literals[@]}"; do
		inherited_open_literals["$key"]=1
		unset 'inherited_close_literals[$key]'
	done
	for key in "${!close_literals[@]}"; do
		inherited_close_literals["$key"]=1
		unset 'inherited_open_literals[$key]'
	done
	for key in "${!show_literals[@]}"; do
		inherited_show_literals["$key"]=1
		unset 'inherited_hide_literals[$key]'
	done
	for key in "${!hide_literals[@]}"; do
		inherited_hide_literals["$key"]=1
		unset 'inherited_show_literals[$key]'
	done

	local pattern
	for pattern in "${open_regexes[@]}"; do
		[[ -z "$pattern" ]] && continue
		inherited_open_regex_map["$pattern"]=1
		unset 'inherited_close_regex_map[$pattern]'
	done
	for pattern in "${close_regexes[@]}"; do
		[[ -z "$pattern" ]] && continue
		inherited_close_regex_map["$pattern"]=1
		unset 'inherited_open_regex_map[$pattern]'
	done
	for pattern in "${show_regexes[@]}"; do
		[[ -z "$pattern" ]] && continue
		inherited_show_regex_map["$pattern"]=1
		unset 'inherited_hide_regex_map[$pattern]'
	done
	for pattern in "${hide_regexes[@]}"; do
		[[ -z "$pattern" ]] && continue
		inherited_hide_regex_map["$pattern"]=1
		unset 'inherited_show_regex_map[$pattern]'
	done
}

load_state_chain() {
	local target="$1"
	reset_inherited_state

	local -a state_dirs=()
	local cursor="$target"
	while true; do
		if [[ -f "$cursor/.dirtree-state" ]]; then
			state_dirs+=("$cursor")
		fi
		[[ "$cursor" == "/" ]] && break
		cursor=$(dirname "$cursor")
	done

	local idx
	for (( idx=${#state_dirs[@]}-1; idx>=0; idx-- )); do
		local state_dir="${state_dirs[$idx]}"
		if [[ "$state_dir" == "$target" ]]; then
			continue
		fi
		read_state "$state_dir"
		merge_inherited_from_current
	done

	read_state "$target"
}

rebuild_effective_state() {
	effective_open_literals=()
	effective_close_literals=()
	effective_show_literals=()
	effective_hide_literals=()
	effective_open_regexes=()
	effective_close_regexes=()
	effective_show_regexes=()
	effective_hide_regexes=()

	effective_default_state=""
	if [[ "$default_state_set" == true && -n "$default_state" ]]; then
		effective_default_state="$default_state"
	elif [[ "$inherited_default_state_set" == true ]]; then
		effective_default_state="$inherited_default_state"
	fi

	effective_default_visibility="shown"
	if [[ "$default_visibility_set" == true && -n "$default_visibility" ]]; then
		effective_default_visibility="$default_visibility"
	elif [[ "$inherited_default_visibility_set" == true ]]; then
		effective_default_visibility="$inherited_default_visibility"
	fi

	effective_depth=""
	if [[ "$persisted_depth_set" == true && -n "$persisted_depth" ]]; then
		effective_depth="$persisted_depth"
	elif [[ "$inherited_depth_set" == true ]]; then
		effective_depth="$inherited_depth"
	fi

	effective_sort_mode=""
	if [[ "$persisted_sort_mode_set" == true && -n "$persisted_sort_mode" ]]; then
		effective_sort_mode="$persisted_sort_mode"
	elif [[ "$inherited_sort_mode_set" == true ]]; then
		effective_sort_mode="$inherited_sort_mode"
	fi

	effective_sort_direction=""
	if [[ "$persisted_sort_direction_set" == true && -n "$persisted_sort_direction" ]]; then
		effective_sort_direction="$persisted_sort_direction"
	elif [[ "$inherited_sort_direction_set" == true ]]; then
		effective_sort_direction="$inherited_sort_direction"
	fi

	effective_color_preference=""
	if [[ "$persisted_color_preference_set" == true && -n "$persisted_color_preference" ]]; then
		effective_color_preference="$persisted_color_preference"
	elif [[ "$inherited_color_preference_set" == true && -n "$inherited_color_preference" ]]; then
		effective_color_preference="$inherited_color_preference"
	fi

	effective_hyperlink_preference=""
	if [[ "$persisted_hyperlink_preference_set" == true && -n "$persisted_hyperlink_preference" ]]; then
		effective_hyperlink_preference="$persisted_hyperlink_preference"
	elif [[ "$inherited_hyperlink_preference_set" == true && -n "$inherited_hyperlink_preference" ]]; then
		effective_hyperlink_preference="$inherited_hyperlink_preference"
	fi

	local key
	for key in "${!inherited_open_literals[@]}"; do
		effective_open_literals["$key"]=1
	done
	for key in "${!inherited_close_literals[@]}"; do
		effective_close_literals["$key"]=1
	done
	for key in "${!inherited_show_literals[@]}"; do
		effective_show_literals["$key"]=1
	done
	for key in "${!inherited_hide_literals[@]}"; do
		effective_hide_literals["$key"]=1
	done

for key in "${!close_literals[@]}"; do
	unset 'effective_open_literals[$key]'
done
for key in "${!open_literals[@]}"; do
	unset 'effective_close_literals[$key]'
done
for key in "${!hide_literals[@]}"; do
	unset 'effective_show_literals[$key]'
done
for key in "${!show_literals[@]}"; do
	unset 'effective_hide_literals[$key]'
done

for key in "${!open_literals[@]}"; do
	effective_open_literals["$key"]=1
done
for key in "${!close_literals[@]}"; do
	effective_close_literals["$key"]=1
done
for key in "${!show_literals[@]}"; do
	effective_show_literals["$key"]=1
done
for key in "${!hide_literals[@]}"; do
	effective_hide_literals["$key"]=1
done

	local -A open_map=()
	local -A close_map=()
	local -A show_map=()
	local -A hide_map=()

	for key in "${!inherited_open_regex_map[@]}"; do
		open_map["$key"]=1
	done
	for key in "${!inherited_close_regex_map[@]}"; do
		close_map["$key"]=1
	done
	for key in "${!inherited_show_regex_map[@]}"; do
		show_map["$key"]=1
	done
	for key in "${!inherited_hide_regex_map[@]}"; do
		hide_map["$key"]=1
	done

	local pattern
for pattern in "${close_regexes[@]}"; do
	[[ -z "$pattern" ]] && continue
	unset 'open_map[$pattern]'
done
for pattern in "${open_regexes[@]}"; do
	[[ -z "$pattern" ]] && continue
	unset 'close_map[$pattern]'
done
for pattern in "${hide_regexes[@]}"; do
	[[ -z "$pattern" ]] && continue
	unset 'show_map[$pattern]'
done
for pattern in "${show_regexes[@]}"; do
	[[ -z "$pattern" ]] && continue
	unset 'hide_map[$pattern]'
done

for pattern in "${open_regexes[@]}"; do
	[[ -z "$pattern" ]] && continue
	open_map["$pattern"]=1
done
for pattern in "${close_regexes[@]}"; do
	[[ -z "$pattern" ]] && continue
	close_map["$pattern"]=1
done
for pattern in "${show_regexes[@]}"; do
	[[ -z "$pattern" ]] && continue
	show_map["$pattern"]=1
done
for pattern in "${hide_regexes[@]}"; do
	[[ -z "$pattern" ]] && continue
	hide_map["$pattern"]=1
done

local state_file_name=".dirtree-state"
local show_state_visible=false
if [[ -n "${effective_show_literals[$state_file_name]:-}" ]]; then
	show_state_visible=true
else
	for pattern in "${!show_map[@]}"; do
		[[ -z "$pattern" ]] && continue
		if regex_token_matches "$pattern" "$state_file_name"; then
			show_state_visible=true
			break
		fi
	done
fi
if [[ "$show_state_visible" != true ]]; then
	hide_map["^\\.dirtree-state$"]=1
fi

if (( ${#open_map[@]} > 0 )); then
	mapfile -t effective_open_regexes < <(printf '%s\n' "${!open_map[@]}" | sort)
fi
	if (( ${#close_map[@]} > 0 )); then
		mapfile -t effective_close_regexes < <(printf '%s\n' "${!close_map[@]}" | sort)
	fi
	if (( ${#show_map[@]} > 0 )); then
		mapfile -t effective_show_regexes < <(printf '%s\n' "${!show_map[@]}" | sort)
	fi
	if (( ${#hide_map[@]} > 0 )); then
		mapfile -t effective_hide_regexes < <(printf '%s\n' "${!hide_map[@]}" | sort)
	fi

	build_regex_tests effective_open_regexes open_regex_tests
	build_regex_tests effective_close_regexes close_regex_tests
	build_regex_tests effective_show_regexes show_regex_tests
	build_regex_tests effective_hide_regexes hide_regex_tests
}

persist_state() {
	local target_dir="$1"; shift
	local state_file="$target_dir/.dirtree-state"
	local tmp_file
	tmp_file=$(mktemp "${state_file}.XXXXXX")

	{
		echo "$STATE_HEADER_COMMENT"
		echo "$STATE_VERSION_LABEL"
		local wrote_block=false
		local default_entries=()
		if [[ "$default_state_set" == true ]]; then
			default_entries+=("$default_state")
		fi
		if [[ "$default_visibility_set" == true ]]; then
			default_entries+=("$default_visibility")
		fi
		if (( ${#default_entries[@]} == 0 )) && [[ -n "$default_state" ]]; then
			default_entries+=("$default_state")
		fi
		if (( ${#default_entries[@]} == 1 )); then
			echo "default=${default_entries[0]}"
			wrote_block=true
		elif (( ${#default_entries[@]} > 1 )); then
			echo "default=["
			for entry in "${default_entries[@]}"; do
				printf '\t%s\n' "$entry"
			done
			echo "]"
			wrote_block=true
		fi

		local -a scalar_lines=()
		if [[ "$persisted_depth_set" == true && -n "$persisted_depth" ]]; then
			scalar_lines+=("depth=$persisted_depth")
		fi
		if [[ "$persisted_sort_mode_set" == true && -n "$persisted_sort_mode" ]]; then
			scalar_lines+=("sort=$persisted_sort_mode")
		fi
		if [[ "$persisted_sort_direction_set" == true && -n "$persisted_sort_direction" ]]; then
			scalar_lines+=("sort_direction=$persisted_sort_direction")
		fi
		if [[ "$persisted_color_preference_set" == true && -n "$persisted_color_preference" ]]; then
			scalar_lines+=("color=$persisted_color_preference")
		fi
		if [[ "$persisted_hyperlink_preference_set" == true && -n "$persisted_hyperlink_preference" ]]; then
			scalar_lines+=("hyperlink=$persisted_hyperlink_preference")
		fi
		if (( ${#scalar_lines[@]} > 0 )); then
			if [[ "$wrote_block" == true ]]; then
				echo
			fi
			printf '%s\n' "${scalar_lines[@]}"
			wrote_block=true
		fi

		local -a entries=()
		local key

		entries=()
		for key in "${!open_literals[@]}"; do
			entries+=("$key")
		done
		for key in "${open_regexes[@]}"; do
			entries+=("$(wrap_regex_pattern "$key")")
		done
		if (( ${#entries[@]} > 0 )); then
			IFS=$'\n' entries=($(printf '%s\n' "${entries[@]}" | sort))
			if [[ "$wrote_block" == true ]]; then
				echo
			fi
			echo "open=["
			for key in "${entries[@]}"; do
				if is_wrapped_regex_string "$key"; then
					if [[ -n "${open_regex_comments[$key]:-}" ]]; then
						printf '%s' "${open_regex_comments[$key]}"
					fi
				else
					if [[ -n "${open_literal_comments[$key]:-}" ]]; then
						printf '%s' "${open_literal_comments[$key]}"
					fi
				fi
				printf '\t%s\n' "$key"
			done
			echo "]"
			wrote_block=true
		fi

		entries=()
		for key in "${!close_literals[@]}"; do
			entries+=("$key")
		done
		for key in "${close_regexes[@]}"; do
			entries+=("$(wrap_regex_pattern "$key")")
		done
		if (( ${#entries[@]} > 0 )); then
			IFS=$'\n' entries=($(printf '%s\n' "${entries[@]}" | sort))
			if [[ "$wrote_block" == true ]]; then
				echo
			fi
			echo "close=["
			for key in "${entries[@]}"; do
				if is_wrapped_regex_string "$key"; then
					if [[ -n "${close_regex_comments[$key]:-}" ]]; then
						printf '%s' "${close_regex_comments[$key]}"
					fi
				else
					if [[ -n "${close_literal_comments[$key]:-}" ]]; then
						printf '%s' "${close_literal_comments[$key]}"
					fi
				fi
				printf '\t%s\n' "$key"
			done
			echo "]"
			wrote_block=true
		fi

		entries=()
		for key in "${!show_literals[@]}"; do
			entries+=("$key")
		done
		for key in "${show_regexes[@]}"; do
			entries+=("$(wrap_regex_pattern "$key")")
		done
		if (( ${#entries[@]} > 0 )); then
			IFS=$'\n' entries=($(printf '%s\n' "${entries[@]}" | sort))
			if [[ "$wrote_block" == true ]]; then
				echo
			fi
			echo "show=["
			for key in "${entries[@]}"; do
				if is_wrapped_regex_string "$key"; then
					if [[ -n "${show_regex_comments[$key]:-}" ]]; then
						printf '%s' "${show_regex_comments[$key]}"
					fi
				else
					if [[ -n "${show_literal_comments[$key]:-}" ]]; then
						printf '%s' "${show_literal_comments[$key]}"
					fi
				fi
				printf '\t%s\n' "$key"
			done
			echo "]"
			wrote_block=true
		fi

		entries=()
		for key in "${!hide_literals[@]}"; do
			entries+=("$key")
		done
		for key in "${hide_regexes[@]}"; do
			entries+=("$(wrap_regex_pattern "$key")")
		done
		if (( ${#entries[@]} > 0 )); then
			IFS=$'\n' entries=($(printf '%s\n' "${entries[@]}" | sort))
			if [[ "$wrote_block" == true ]]; then
				echo
			fi
			echo "hide=["
			for key in "${entries[@]}"; do
				if is_wrapped_regex_string "$key"; then
					if [[ -n "${hide_regex_comments[$key]:-}" ]]; then
						printf '%s' "${hide_regex_comments[$key]}"
					fi
				else
					if [[ -n "${hide_literal_comments[$key]:-}" ]]; then
						printf '%s' "${hide_literal_comments[$key]}"
					fi
				fi
				printf '\t%s\n' "$key"
			done
			echo "]"
			wrote_block=true
		fi

		if (( ${#state_passthrough_lines[@]} > 0 )); then
			if [[ "$wrote_block" == true ]]; then
				echo
			fi
			printf '%s\n' "${state_passthrough_lines[@]}"
		fi
	} >"$tmp_file"

	mv -f "$tmp_file" "$state_file"
	state_needs_migration=false
}


dir_has_children() {
	local d=${1:-.}
	if [[ -n "${dir_children_cache[$d]:-}" ]]; then
		[[ "${dir_children_cache[$d]}" -eq 1 ]]
		return
	fi
	local reset_nullglob=0
	local reset_dotglob=0
	if ! shopt -q nullglob; then
		shopt -s nullglob
		reset_nullglob=1
	fi
	if ! shopt -q dotglob; then
		shopt -s dotglob
		reset_dotglob=1
	fi
	local entries=("$d"/* "$d"/.[!.]* "$d"/..?*)
	if (( reset_nullglob == 1 )); then
		shopt -u nullglob
	fi
	if (( reset_dotglob == 1 )); then
		shopt -u dotglob
	fi
	local has_child=0
	for entry in "${entries[@]}"; do
		if [[ -e "$entry" ]]; then
			has_child=1
			break
		fi
	done
	dir_children_cache["$d"]=$has_child
	if (( has_child )); then
		return 0
	fi
	return 1
}

path_state() {
	local rel="$1"
	open_match_type="none"
	open_match_pattern=""
	close_match_type="none"
	close_match_pattern=""
	show_match_type="none"
	show_match_pattern=""
	hide_match_type="none"
	hide_match_pattern=""

	if [[ -n "$rel" && -n "${effective_open_literals[$rel]:-}" ]]; then
		open_match_type="literal"
		open_match_pattern="$rel"
	else
		for spec in "${open_regex_tests[@]}"; do
			[[ -z "$spec" ]] && continue
			IFS=$REGEX_SPEC_SEP read -r neg pattern <<<"$spec"
			[[ -z "$pattern" ]] && continue
			if (( neg == 0 )); then
				if [[ "$rel" =~ $pattern ]]; then
					open_match_type="regex"
					open_match_pattern="$pattern"
					break
				fi
			else
				if [[ ! "$rel" =~ $pattern ]]; then
					open_match_type="regex"
					open_match_pattern="!$pattern"
					break
				fi
			fi
		done
	fi

	if [[ -n "$rel" && -n "${effective_close_literals[$rel]:-}" ]]; then
		close_match_type="literal"
		close_match_pattern="$rel"
	else
		for spec in "${close_regex_tests[@]}"; do
			[[ -z "$spec" ]] && continue
			IFS=$REGEX_SPEC_SEP read -r neg pattern <<<"$spec"
			[[ -z "$pattern" ]] && continue
			if (( neg == 0 )); then
				if [[ "$rel" =~ $pattern ]]; then
					close_match_type="regex"
					close_match_pattern="$pattern"
					break
				fi
			else
				if [[ ! "$rel" =~ $pattern ]]; then
					close_match_type="regex"
					close_match_pattern="!$pattern"
					break
				fi
			fi
		done
	fi

	if [[ -n "$rel" && -n "${effective_show_literals[$rel]:-}" ]]; then
		show_match_type="literal"
		show_match_pattern="$rel"
	else
		for spec in "${show_regex_tests[@]}"; do
			[[ -z "$spec" ]] && continue
			IFS=$REGEX_SPEC_SEP read -r neg pattern <<<"$spec"
			[[ -z "$pattern" ]] && continue
			if (( neg == 0 )); then
				if [[ "$rel" =~ $pattern ]]; then
					show_match_type="regex"
					show_match_pattern="$pattern"
					break
				fi
			else
				if [[ ! "$rel" =~ $pattern ]]; then
					show_match_type="regex"
					show_match_pattern="!$pattern"
					break
				fi
			fi
		done
	fi

	if [[ -n "$rel" && -n "${effective_hide_literals[$rel]:-}" ]]; then
		hide_match_type="literal"
		hide_match_pattern="$rel"
	else
		for spec in "${hide_regex_tests[@]}"; do
			[[ -z "$spec" ]] && continue
			IFS=$REGEX_SPEC_SEP read -r neg pattern <<<"$spec"
			[[ -z "$pattern" ]] && continue
			if (( neg == 0 )); then
				if [[ "$rel" =~ $pattern ]]; then
					hide_match_type="regex"
					hide_match_pattern="$pattern"
					break
				fi
			else
				if [[ ! "$rel" =~ $pattern ]]; then
					hide_match_type="regex"
					hide_match_pattern="!$pattern"
					break
				fi
			fi
		done
	fi
}

evaluate_path() {
	local rel="$1" parent_closed="$2"
	path_state "$rel"
	path_hidden=0

	local open_type="$open_match_type"
	local close_type="$close_match_type"
	local open_pat="$open_match_pattern"
	local close_pat="$close_match_pattern"
	local show_type="$show_match_type"
	local hide_type="$hide_match_type"
	local show_pat="$show_match_pattern"
	local hide_pat="$hide_match_pattern"

	if [[ "$show_hidden" == true ]]; then
		hide_type="none"
		hide_pat=""
	fi

	if [[ "$open_type" != "none" && "$close_type" != "none" ]]; then
		if [[ "$open_type" == "literal" && "$close_type" == "regex" ]]; then
			close_type="none"
			close_pat=""
		elif [[ "$open_type" == "regex" && "$close_type" == "literal" ]]; then
			open_type="none"
			open_pat=""
		else
			echo "Error: path '$rel' matches both open and close patterns" >&2
			[[ -n "$open_pat" ]] && echo "  open pattern: $open_pat" >&2
			[[ -n "$close_pat" ]] && echo "  close pattern: $close_pat" >&2
			exit 1
		fi
	fi

	if [[ "$show_type" != "none" && "$hide_type" != "none" ]]; then
		if [[ "$show_type" == "literal" && "$hide_type" == "regex" ]]; then
			hide_type="none"
			hide_pat=""
		elif [[ "$show_type" == "regex" && "$hide_type" == "literal" ]]; then
			show_type="none"
			show_pat=""
		else
			echo "Error: path '$rel' matches both show and hide patterns" >&2
			[[ -n "$show_pat" ]] && echo "  show pattern: $show_pat" >&2
			[[ -n "$hide_pat" ]] && echo "  hide pattern: $hide_pat" >&2
			exit 1
		fi
	fi

	if [[ "$show_hidden" == true ]]; then
		path_hidden=0
	elif [[ "$hide_type" != "none" ]]; then
		path_hidden=1
	elif [[ "$show_type" != "none" ]]; then
		path_hidden=0
	else
		path_hidden=0
	fi

	if [[ "$parent_closed" == "1" ]]; then
		path_closed=1
	elif [[ "$close_type" != "none" ]]; then
		path_closed=1
	elif [[ "$open_type" != "none" ]]; then
		path_closed=0
	elif [[ "$effective_default_state" == "closed" ]]; then
		path_closed=1
	else
		path_closed=0
	fi

	if [[ "$priority_enabled" == true && -n "$rel" ]]; then
		if [[ -n "${priority_dirs[$rel]:-}" ]]; then
			path_hidden=0
			path_closed=0
		fi
		if [[ -n "${priority_files[$rel]:-}" ]]; then
			path_hidden=0
		fi
	fi
}

# Append markers while preserving trailing ANSI resets and OSC8 terminators
append_with_marker() {
	local line="$1" marker="$2"
	local osc_close=$'\e]8;;'
	local st=$'\e\\'
	local trailing_hyper=""
	if [[ "$line" == *"$osc_close$st" ]]; then
		trailing_hyper="$osc_close$st"
		line="${line%$osc_close$st}"
	elif [[ "$line" == *"$st" ]]; then
		trailing_hyper="$st"
		line="${line%$st}"
	fi
	local trailing_color=""
	while [[ "$line" =~ (.*)(\x1b\[[0-9;]*m)$ ]]; do
		line="${BASH_REMATCH[1]}"
		trailing_color="${BASH_REMATCH[2]}${trailing_color}"
	done
	printf '%s' "${line}${marker}${trailing_color}${trailing_hyper}"
}

print_decorated_root_header() {
	local color="${eza_color:-always}"
	local icons="${eza_icons:-always}"
	local eza_args=(-d --hyperlink --icons="$icons" --color="$color" --git -1 "$abs_dir")
	local line
	if ! line=$(eza "${eza_args[@]}"); then
		return 1
	fi
	line=$(append_with_marker "$line" "/")
	printf '%s\n' "$line"
}

# Recursively render decorated output without traversing closed subtrees
render_dir() {
	local rel="$1" depth_left="$2" parent_closed="$3" prefix="$4"
	local abs
	if [[ -z "$rel" ]]; then abs="$abs_dir"; else abs="$abs_dir/$rel"; fi
	local eza_args=(--hyperlink --icons="${eza_icons}" --color="${eza_color}" --git --sort "${render_sort_field}" --all -1)
	if [[ "$render_sort_reverse" -eq 1 ]]; then
		eza_args+=(--reverse)
	fi
	local hp=$'\e]8;;file://'
	local st=$'\e\\'
	local lines=()
	local rels=()
	local targets=()
	local isdirs=()
	while IFS= read -r line; do
		local decoded_target=""
		if [[ "$line" == *"$hp"* ]]; then
			local rest="${line#*"$hp"}"
			local target="${rest%%"$st"*}"
			decoded_target=$(osc8_decode_path "$target")
		fi
		lines+=("$line")
		if [[ -n "$decoded_target" ]]; then
			local child_rel
			child_rel=$(normalize_rel "$abs_dir" "$decoded_target")
			rels+=("$child_rel")
			targets+=("$decoded_target")
			local is_dir_val=0
			if [[ -d "$decoded_target" ]]; then
				is_dir_val=1
			fi
			isdirs+=("$is_dir_val")
		else
			rels+=("")
			targets+=("")
			isdirs+=(0)
		fi
	done < <(eza "${eza_args[@]}" "$abs")

	local count=${#lines[@]}
	local -a visible_indices=()
	local -a cached_closed=()
	local idx
	for ((idx=0; idx<count; idx++)); do
		local child_rel="${rels[$idx]}"
		local target="${targets[$idx]}"
		if [[ -z "$child_rel" ]]; then
			rels[$idx]="$child_rel"
			if [[ -n "$target" ]]; then
				targets[$idx]="$target"
			else
				if [[ -n "$child_rel" ]]; then
					targets[$idx]="$abs/$child_rel"
				else
					targets[$idx]="$abs"
				fi
			fi
		fi
		if [[ -n "$child_rel" && "$child_rel" != "." && "$child_rel" != ".." ]]; then
			evaluate_path "$child_rel" "$parent_closed"
			cached_closed[$idx]=$path_closed
		if [[ $path_hidden -eq 1 ]]; then
			if [[ "${isdirs[$idx]}" -eq 1 ]]; then
				note_hidden dir "$child_rel"
			else
				note_hidden file "$child_rel"
			fi
			continue
		fi
		else
			cached_closed[$idx]=0
		fi
		visible_indices+=("$idx")
	done

	local visible_count=${#visible_indices[@]}
	local pos
	for ((pos=0; pos<visible_count; pos++)); do
		idx=${visible_indices[$pos]}
		local line="${lines[$idx]}"
		local child_rel="${rels[$idx]}"
		local target="${targets[$idx]}"
		local is_dir="${isdirs[$idx]}"
		local path_closed_val=${cached_closed[$idx]:-0}
		if [[ -z "$target" && -n "$child_rel" ]]; then
			target="$abs/$child_rel"
		fi
		local is_last=0
		if (( pos == visible_count-1 )); then
			is_last=1
		fi
		local connector="├── "
		local next_prefix="${prefix}│   "
		if (( is_last )); then
			connector="└── "
			next_prefix="${prefix}    "
		fi
		if [[ -n "$child_rel" && "$child_rel" != "." && "$child_rel" != ".." && "$is_dir" -eq 1 ]]; then
			local marker="/"
			if (( path_closed_val == 1 )); then
				if [[ -n "$target" ]] && dir_has_children "$target"; then
					marker="/*"
				fi
			fi
			line=$(append_with_marker "$line" "$marker")
			printf '%s%s%s\n' "$prefix" "$connector" "$line"
			if (( path_closed_val == 0 && depth_left > 1 )); then
				render_dir "$child_rel" $((depth_left-1)) "$path_closed_val" "$next_prefix"
			fi
		else
			printf '%s%s%s\n' "$prefix" "$connector" "$line"
		fi
	done
}

# Parse command line arguments
while [[ $# -gt 0 ]]; do
	case "$1" in
		-h|--help)
			show_help
			exit 0
			;;
		-a|--about)
		echo "Stateful eza tree (icons/colors/links); --simple for LLMs; persists .dirtree-state (default/open/close/show/hide); regex via /pattern/ or !/pattern/; literals must be relative; env: DIRTREE_{SIMPLE,DECORATED,AUTO_SIMPLE}."
			exit 0
			;;
		-d|--depth)
			shift
			if [[ -n "$1" && "$1" =~ ^[0-9]+$ ]]; then
				depth="$1"
				cli_depth_override=true
				cli_depth_value="$1"
			else
				echo "Error: --depth requires a numeric argument" >&2
				exit 1
			fi
			;;
		--simple)
			simple_mode=true
			;;
	--decorated)
		force_decorated=true
		;;
	--no-icons)
		user_disabled_icons=true
		;;
	--no-color)
		cli_color_override=true
		cli_color_value="false"
		;;
	--no-hyperlinks)
		cli_hyperlink_override=true
		cli_hyperlink_value="false"
		;;
	--default)
			shift
			collect_variadic_args collected "open opened close closed show shown hide hidden" "$@"
			if (( variadic_arg_count == 0 )); then
				echo "Error: --default requires at least one value" >&2
				exit 1
			fi
			for token in "${collected[@]}"; do
				local_token="${token,,}"
				case "$local_token" in
					open|opened)
						if [[ -n "$cli_default_state" && "$cli_default_state" != "opened" ]]; then
							echo "Error: --default state conflict between '$cli_default_state' and '$token'" >&2
							exit 1
						fi
						cli_default_state="opened"
						;;
					close|closed)
						if [[ -n "$cli_default_state" && "$cli_default_state" != "closed" ]]; then
							echo "Error: --default state conflict between '$cli_default_state' and '$token'" >&2
							exit 1
						fi
						cli_default_state="closed"
						;;
					show|shown)
						if [[ -n "$cli_default_visibility" && "$cli_default_visibility" != "shown" ]]; then
							echo "Error: --default visibility conflict between '$cli_default_visibility' and '$token'" >&2
							exit 1
						fi
						cli_default_visibility="shown"
						;;
					hide|hidden)
						if [[ -n "$cli_default_visibility" && "$cli_default_visibility" != "hidden" ]]; then
							echo "Error: --default visibility conflict between '$cli_default_visibility' and '$token'" >&2
							exit 1
						fi
						cli_default_visibility="hidden"
						;;
					*)
						echo "Error: --default accepts opened/closed/shown/hidden" >&2
						exit 1
						;;
				esac
			done
			shift "$variadic_arg_count"
			state_modified=true
			unset collected
			continue
			;;
		--sort)
				shift
				case "$1" in
					alpha|modified)
						sort_mode="$1"
						cli_sort_mode_override=true
						cli_sort_mode_value="$1"
						;;
					*) echo "Error: --sort requires 'modified' or 'alpha'" >&2; exit 1 ;;
				esac
				;;
			--asc)
				sort_reverse=0
				cli_sort_direction_override=true
				cli_sort_direction_value="asc"
				;;
			--desc)
				sort_reverse=1
				cli_sort_direction_override=true
				cli_sort_direction_value="desc"
				;;
			--show-hidden)
				show_hidden=true
				;;
			--rewrite-settings)
				rewrite_settings=true
				;;
		-o|--open)
			shift
			collect_variadic_args collected "" "$@"
			if (( variadic_arg_count == 0 )); then
				echo "Error: --open requires at least one directory" >&2
				exit 1
			fi
			for entry in "${collected[@]}"; do
				pattern_val=""
					parse_wrapped_regex_token "$entry" pattern_val
					case $? in
						0)
							open_regex_args+=("$pattern_val")
							;;
						2)
							echo "Error: --open regex pattern must not be empty" >&2
							exit 1
							;;
						*)
							if is_glob_pattern "$entry"; then
								regex_value=$(glob_to_regex "$entry")
								open_regex_args+=("$regex_value")
							else
								open_args+=("$entry")
							fi
							;;
				esac
			done
			shift "$variadic_arg_count"
			state_modified=true
			unset collected
			continue
			;;
		-c|--close)
			shift
			collect_variadic_args collected "" "$@"
			if (( variadic_arg_count == 0 )); then
				echo "Error: --close requires at least one directory" >&2
				exit 1
			fi
			for entry in "${collected[@]}"; do
				pattern_val=""
					parse_wrapped_regex_token "$entry" pattern_val
					case $? in
						0)
							close_regex_args+=("$pattern_val")
							;;
						2)
							echo "Error: --close regex pattern must not be empty" >&2
							exit 1
							;;
						*)
							if is_glob_pattern "$entry"; then
								regex_value=$(glob_to_regex "$entry")
								close_regex_args+=("$regex_value")
							else
								close_args+=("$entry")
							fi
							;;
				esac
			done
			shift "$variadic_arg_count"
			state_modified=true
			unset collected
			continue
			;;
		--show)
			shift
			collect_variadic_args collected "" "$@"
			if (( variadic_arg_count == 0 )); then
				echo "Error: --show requires at least one path" >&2
				exit 1
			fi
			for entry in "${collected[@]}"; do
				pattern_val=""
					parse_wrapped_regex_token "$entry" pattern_val
					case $? in
						0)
							show_regex_args+=("$pattern_val")
							;;
						2)
							echo "Error: --show regex pattern must not be empty" >&2
							exit 1
							;;
						*)
							if [[ "$entry" == /* ]]; then
								echo "Error: --show paths must be relative (no leading '/'): $entry" >&2
								exit 1
							fi
							if is_glob_pattern "$entry"; then
								regex_value=$(glob_to_regex "$entry")
								show_regex_args+=("$regex_value")
							else
								show_args+=("$entry")
							fi
							;;
				esac
			done
			shift "$variadic_arg_count"
			state_modified=true
			unset collected
			continue
			;;
		--hide)
			shift
			collect_variadic_args collected "" "$@"
			if (( variadic_arg_count == 0 )); then
				echo "Error: --hide requires at least one path" >&2
				exit 1
			fi
			for entry in "${collected[@]}"; do
				pattern_val=""
					parse_wrapped_regex_token "$entry" pattern_val
					case $? in
						0)
							hide_regex_args+=("$pattern_val")
							;;
						2)
							echo "Error: --hide regex pattern must not be empty" >&2
							exit 1
							;;
						*)
							if [[ "$entry" == /* ]]; then
								echo "Error: --hide paths must be relative (no leading '/'): $entry" >&2
								exit 1
							fi
							if is_glob_pattern "$entry"; then
								regex_value=$(glob_to_regex "$entry")
								hide_regex_args+=("$regex_value")
							else
								hide_args+=("$entry")
							fi
							;;
				esac
			done
			shift "$variadic_arg_count"
			state_modified=true
			unset collected
			continue
			;;
		--test)
			test_bin="${DIRTREE_TEST_BIN:-"$DIRTREE_ROOT/test/dirtree_test"}"
			if [[ -x "$test_bin" ]]; then
				"$test_bin" >/dev/null
				test_status=$?
				exit $test_status
			else
				echo "Error: dirtree_test not found or not executable ($test_bin)." >&2
				exit 1
			fi
			;;
		-*)
			echo "Unknown option: $1" >&2
			exit 1
			;;
		*)
			# This is the directory argument
			dir="$1"
			dir_pending=false
			break
			;;
	esac
	shift
done

# Validate directory exists
if [[ ! -d "$dir" ]]; then
	echo "Error: '$dir' is not a directory" >&2
	exit 1
fi

# Absolute path for rendering
abs_dir=$(cd "$dir" && pwd -P)

if [[ -z "${DIRTREE_SCM_CHANGES_STAY_HIDDEN_OR_CLOSED:-}" ]]; then
	collect_priority_paths "$abs_dir"
else
	case "${DIRTREE_SCM_CHANGES_STAY_HIDDEN_OR_CLOSED}" in
		1|true|TRUE|on|ON)
			:
			;;
		*)
			collect_priority_paths "$abs_dir"
			;;
	esac
fi

# Load and possibly update state (including inheritance)
load_state_chain "$abs_dir"

if [[ "$rewrite_settings" == true && -f "$abs_dir/.dirtree-state" ]]; then
	state_needs_migration=true
fi

# Apply CLI default after loading persisted state so it isn't overwritten
if [[ -n "$cli_default_state" ]]; then
	default_state="$cli_default_state"
	default_state_set=true
fi
if [[ -n "$cli_default_visibility" ]]; then
	default_visibility="$cli_default_visibility"
	default_visibility_set=true
fi

# Apply CLI open/close mutations if any
for pattern in "${open_regex_args[@]}"; do
	append_unique_regex open_regexes "$pattern"
done
for pattern in "${close_regex_args[@]}"; do
	append_unique_regex close_regexes "$pattern"
done
for pattern in "${show_regex_args[@]}"; do
	append_unique_regex show_regexes "$pattern"
done
for pattern in "${hide_regex_args[@]}"; do
	append_unique_regex hide_regexes "$pattern"
done


for p in "${open_args[@]}"; do
	p=$(normalize_rel "$abs_dir" "$p")
	[[ -z "$p" ]] && continue
	if [[ -d "$abs_dir/$p" ]]; then
		open_literals["$p"]=1
		unset 'close_literals[$p]'
	else
		echo "Warning: open target not a directory: $p" >&2
	fi
done

for p in "${close_args[@]}"; do
	p=$(normalize_rel "$abs_dir" "$p")
	[[ -z "$p" ]] && continue
	if [[ -d "$abs_dir/$p" ]]; then
		close_literals["$p"]=1
		unset 'open_literals[$p]'
	else
		echo "Warning: close target not a directory: $p" >&2
	fi
done
for p in "${show_args[@]}"; do
	p=$(normalize_rel "$abs_dir" "$p")
	[[ -z "$p" ]] && continue
	show_literals["$p"]=1
	unset 'hide_literals[$p]'
done
for p in "${hide_args[@]}"; do
	p=$(normalize_rel "$abs_dir" "$p")
	[[ -z "$p" ]] && continue
	hide_literals["$p"]=1
	unset 'show_literals[$p]'
done

if [[ "$cli_depth_override" == true ]]; then
	if [[ "$persisted_depth_set" != true || "$persisted_depth" != "$cli_depth_value" ]]; then
		persisted_depth="$cli_depth_value"
		persisted_depth_set=true
		state_modified=true
	fi
fi
if [[ "$cli_sort_mode_override" == true ]]; then
	if [[ "$persisted_sort_mode_set" != true || "$persisted_sort_mode" != "$cli_sort_mode_value" ]]; then
		persisted_sort_mode="$cli_sort_mode_value"
		persisted_sort_mode_set=true
		state_modified=true
	fi
fi
if [[ "$cli_sort_direction_override" == true ]]; then
	if [[ "$persisted_sort_direction_set" != true || "$persisted_sort_direction" != "$cli_sort_direction_value" ]]; then
		persisted_sort_direction="$cli_sort_direction_value"
		persisted_sort_direction_set=true
		state_modified=true
	fi
fi

if [[ "$cli_color_override" == true ]]; then
	if [[ "$persisted_color_preference_set" != true || "$persisted_color_preference" != "$cli_color_value" ]]; then
		persisted_color_preference="$cli_color_value"
		persisted_color_preference_set=true
		state_modified=true
	fi
fi

if [[ "$cli_hyperlink_override" == true ]]; then
	if [[ "$persisted_hyperlink_preference_set" != true || "$persisted_hyperlink_preference" != "$cli_hyperlink_value" ]]; then
		persisted_hyperlink_preference="$cli_hyperlink_value"
		persisted_hyperlink_preference_set=true
		state_modified=true
	fi
fi

rebuild_effective_state

if [[ "$cli_depth_override" != true && -n "$effective_depth" ]]; then
	depth="$effective_depth"
fi
if [[ "$cli_sort_mode_override" != true && -n "$effective_sort_mode" ]]; then
	sort_mode="$effective_sort_mode"
fi
if [[ "$cli_sort_direction_override" != true ]]; then
	case "$effective_sort_direction" in
		asc) sort_reverse=0 ;;
		desc) sort_reverse=1 ;;
	esac
fi

if [[ "$cli_color_override" == true ]]; then
	color_pref_override="$cli_color_value"
elif [[ -n "$effective_color_preference" ]]; then
	color_pref_override="$effective_color_preference"
else
	color_pref_override=""
fi

if [[ "$cli_hyperlink_override" == true ]]; then
	hyperlink_pref_override="$cli_hyperlink_value"
elif [[ -n "$effective_hyperlink_preference" ]]; then
	hyperlink_pref_override="$effective_hyperlink_preference"
else
	hyperlink_pref_override=""
fi

if [[ "$state_modified" == true || "$state_needs_migration" == true ]]; then
	persist_state "$abs_dir"
fi

case "$sort_mode" in
	alpha) render_sort_field="name" ;;
	*) render_sort_field="modified" ;;
esac
render_sort_reverse="$sort_reverse"

if stdout_is_tty; then
	stdout_is_tty_cached=true
else
	stdout_is_tty_cached=false
fi

# Determine mode if not explicitly forced
if [[ "$force_decorated" == false ]]; then
	if [[ "$simple_mode" != true ]]; then
		if [[ -n "${DIRTREE_SIMPLE:-}" ]]; then
			simple_mode=true
		elif [[ -n "${DIRTREE_DECORATED:-}" ]]; then
			simple_mode=false
			force_decorated=true
		elif [[ -n "${DIRTREE_AUTO_SIMPLE:-}" ]]; then
			# Auto: enable simple when stdout is not a TTY
			if [[ "$stdout_is_tty_cached" == false ]]; then
				simple_mode=true
			fi
		fi
	fi
fi

hidden_count_log=$(mktemp)

# For decorated mode, ensure eza is available
if ! command -v eza >/dev/null 2>&1; then
	if [[ -n "$hidden_count_log" ]]; then
		rm -f "$hidden_count_log"
		hidden_count_log=""
	fi
	echo "Error: eza is not installed. Please install eza first." >&2
	exit 1
fi

# Decide eza presentation based on mode/TTY/preferences.
eza_color="always"
eza_icons="always"
eza_hyperlinks_enabled=false
if [[ "$simple_mode" == true ]]; then
	eza_color="never"
	eza_hyperlinks_enabled=false
	if [[ "$user_disabled_icons" == true ]]; then
		eza_icons="never"
	fi
else
	if [[ "$force_decorated" == true || "$stdout_is_tty_cached" == true ]]; then
		eza_color="always"
		eza_hyperlinks_enabled=true
	else
		eza_color="never"
		eza_hyperlinks_enabled=false
	fi
	if [[ "$user_disabled_icons" == true ]]; then
		eza_icons="never"
	fi
fi

if [[ "$color_pref_override" == "false" ]]; then
	eza_color="never"
elif [[ "$color_pref_override" == "true" && "$simple_mode" != true ]]; then
	eza_color="always"
fi

if [[ "$hyperlink_pref_override" == "false" || "$simple_mode" == true ]]; then
	eza_hyperlinks_enabled=false
elif [[ "$hyperlink_pref_override" == "true" && "$simple_mode" != true ]]; then
	eza_hyperlinks_enabled=true
fi

strip_osc8=false
if [[ "$eza_hyperlinks_enabled" != true ]]; then
	strip_osc8=true
fi

strip_osc8_links() {
	perl -CSDA -0pe 's/\e]8;;.*?\e\\//gs'
}

# Run renderer with optional OSC8 stripping
status=0
if $strip_osc8; then
	(
		set -o pipefail
		{
			print_decorated_root_header
			render_dir "" "$depth" 0 ""
		} | strip_osc8_links
	) || status=$?
else
	if ! print_decorated_root_header; then
		status=$?
	else
		render_dir "" "$depth" 0 ""
		status=$?
	fi
fi

report_hidden_counts
if [[ -n "$hidden_count_log" ]]; then
	rm -f "$hidden_count_log"
	hidden_count_log=""
fi

exit $status
