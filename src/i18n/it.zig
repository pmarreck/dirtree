...
pub const strings = Strings{
    ...
    .help_opt_temp = "  -t, --temp         Applica le modifiche solo per questa esecuzione (non salvato)",
    .help_opt_persist = "  --persist, --save  Salva anche queste impostazioni (sostituisce non-TTY e DIRTREE_TEMP)",
    ...
    .help_opt_no_color = "  --no-color        Abilita/disabilita i colori ANSI (persistente)",
    ...
    .help_opt_no_hyperlinks = "  --no-hyperlinks   Abilita/disabilita i collegamenti OSC8 (persistente)",
    ...
    .help_behavior_text = "Le impostazioni della presentazione vengono salvate quando stdout è un terminale; altrimenti si applicano solo all'invocazione corrente. Per impostazione predefinita, le modifiche --open/--close/--show/--hide vengono sempre salvate. Il colore è attivo per impostazione predefinita per l'output del terminale e disattivato per gli altri output. --temp o --persist/--save sovrascrivono esplicitamente queste regole di salvataggio.",
    .persistence_note_tty = "Messaggio: {s}: salvato perché stdout è un terminale; utilizzare --temp per applicarlo solo a questa invocazione.",
    .persistence_note_non_tty = "Messaggio: {s}: non salvato perché stdout non è un terminale; utilizzare --persist/--save per sovrascrivere.",
    .persistence_note_semantic = "Messaggio: {s}: salvato perché le modifiche alla vista del progetto condiviso vengono salvate per impostazione predefinita; utilizzare --temp per applicarli solo a questa invocazione.",
    .persistence_note_env = "Messaggio: {s}: non salvato perché DIRTREE_TEMP=1; utilizzare --persist/--save per sovrascrivere.",
    .persistence_note_mute = "Impostare DIRTREE_MUTE_PERSISTENCE_REASON=1 per sopprimere questo messaggio informativo.",
    ...
    .help_opt_annotate = "  annotate PATH DESC Salva una nota di una riga su un file o directory (alias: note; DESC vuoto cancella)",
...
pub const aliases = LocaleAliases{
    .cli = &[_]CliAliasEntry{
        ...
        .{ .name = "--collegamenti", .arg = .hyperlinks },
