...
pub const strings = Strings{
    ...
    .help_opt_temp = "  -t, --temp         تطبيق التغييرات لهذا التشغيل فقط (لا يُحفظ)",
    .help_opt_persist = "  --persist, --save  احفظ هذه الإعدادات أيضًا (يتجاوز non-TTY وDIRTREE_TEMP)",
    ...
    .help_opt_no_color = "  --no-color        تفعيل/تعطيل ألوان ANSI (محفوظ)",
    ...
    .help_opt_no_hyperlinks = "  --no-hyperlinks   تفعيل/تعطيل روابط OSC8 (محفوظ)",
    ...
    .help_behavior_text = "يتم حفظ إعدادات العرض التقديمي عندما يكون stdout محطة طرفية؛ وإلا فإنها تنطبق فقط على الاحتجاج الحالي. بشكل افتراضي، يتم دائمًا حفظ تغييرات --open/--close/--show/--hide. يتم تشغيل اللون بشكل افتراضي للمخرجات الطرفية ويتم إيقاف تشغيله للمخرجات الأخرى. يتجاوز --temp أو --persist/--save قواعد الحفظ هذه بشكل صريح.",
    .persistence_note_tty = "الرسالة: {s}: تم حفظها لأن stdout محطة طرفية؛ استخدم --temp لتطبيقه على هذا الاستدعاء فقط.",
    .persistence_note_non_tty = "الرسالة: {s}: لم يتم حفظها لأن stdout ليس محطة طرفية؛ استخدم --persist/--save للتجاوز.",
    .persistence_note_semantic = "الرسالة: {s}: تم الحفظ لأنه يتم حفظ التغييرات التي تم إجراؤها على عرض المشروع المشترك بشكل افتراضي؛ استخدم --temp لتطبيقها على هذا الاستدعاء فقط.",
    .persistence_note_env = "الرسالة: {s}: لم يتم حفظها لأن DIRTREE_TEMP=1؛ استخدم --persist/--save للتجاوز.",
    .persistence_note_mute = "قم بتعيين DIRTREE_MUTE_PERSISTENCE_REASON=1 لمنع هذه الرسالة الإعلامية.",
    ...
    .err_path_requires_arg = "خطأ: --path يتطلب وسيطة دليل (en: Error: --path requires a directory argument)",
    ...
    .err_only_requires_path = "خطأ: --only يتطلب وسيطة مسار (en: Error: --only requires a path argument)",
    .err_max_lines_requires_number = "خطأ: --max-lines يتطلب معاملاً رقمياً (en: Error: --max-lines requires a numeric argument)",
    .err_notes_requires_mode = "خطأ: --notes يتطلب 'aligned' أو 'inline' (en: Error: --notes requires 'aligned' or 'inline')",
    .err_sort_requires_mode = "خطأ: --sort يتطلب 'modified' أو 'alpha' (en: Error: --sort requires 'modified' or 'alpha')",
    .err_default_requires_value = "خطأ: --default يتطلب قيمة واحدة على الأقل (en: Error: --default requires at least one value)",
    .err_default_state_conflict = "خطأ: تعارض في حالة --default (en: Error: --default state conflict)",
    .err_default_accepts = "خطأ: --default يقبل opened/closed (en: Error: --default accepts opened/closed)",
    .err_open_requires_dir = "خطأ: --open يتطلب مجلداً واحداً على الأقل (en: Error: --open requires at least one directory)",
    .err_close_requires_dir = "خطأ: --close يتطلب مجلداً واحداً على الأقل (en: Error: --close requires at least one directory)",
    .err_show_requires_path = "خطأ: --show يتطلب مساراً واحداً على الأقل (en: Error: --show requires at least one path)",
    .err_hide_requires_path = "خطأ: --hide يتطلب مساراً واحداً على الأقل (en: Error: --hide requires at least one path)",
    .err_unknown_option = "خيار غير معروف (en: Unknown option)",
    .err_not_a_directory = "خطأ: '{s}' ليس مجلداً (en: Error: '{s}' is not a directory)",
    .err_regex_empty = "خطأ: نمط التعبير النمطي يجب ألا يكون فارغاً (en: Error: regex pattern must not be empty)",
    .err_paths_must_be_relative = "خطأ: مسارات {s} يجب أن تكون نسبية (بدون '/' في البداية): {s} (en: Error: {s} paths must be relative (no leading '/'): {s})",
    .err_out_of_memory = "نفدت الذاكرة (en: Out of memory)",
    .err_regex_conflict_path = "خطأ: المسار '{s}' يطابق أنماط الفتح والإغلاق معاً (en: Error: path '{s}' matches both open and close patterns)",
    .err_regex_conflict_open = "  نمط الفتح: {s} (en:   open pattern: {s})",
    .err_regex_conflict_close = "  نمط الإغلاق: {s} (en:   close pattern: {s})",
    .err_regex_invalid = "خطأ: نمط تعبير نمطي غير صالح: {s} (en: Error: invalid regex pattern: {s})",
    .err_unknown_lang = "خطأ: رمز لغة غير معروف '{s}'. المتاحة: {s} (en: Error: unknown language code '{s}'. Available: {s})",
    ...
    .help_opt_annotate = "  annotate PATH DESC حفظ ملاحظة من سطر واحد حول ملف أو مجلد (الاسم المستعار: note؛ الملاحظة الفارغة تمسحها)",
    ...
    // ── متفرقات ──────────────────────────────────────────────
    .err_test_bin_run = "خطأ: تعذّر تشغيل DIRTREE_TEST_BIN: {s} (en: Error: could not run DIRTREE_TEST_BIN: {s})",
    .err_test_bin_wait = "خطأ: تعذّر انتظار DIRTREE_TEST_BIN (en: Error: could not wait for DIRTREE_TEST_BIN)",
    .err_render_tree = "خطأ في رسم الشجرة: {} (en: Error rendering tree: {})",
};
