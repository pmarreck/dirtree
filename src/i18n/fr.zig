...
pub const strings = Strings{
    ...
    .help_opt_temp = "  -t, --temp         Appliquer les changements pour cette exécution seulement (non enregistré)",
    .help_opt_persist = "  --persist, --save  Enregistrez également ces paramètres (remplace non-TTY et DIRTREE_TEMP)",
    ...
    .help_opt_no_color = "  --no-color        Activer/désactiver les couleurs ANSI (persistant)",
    ...
    .help_opt_no_hyperlinks = "  --no-hyperlinks   Activer/désactiver les hyperliens OSC8 (persistant)",
    ...
    .help_behavior_text = "Les paramètres de présentation sont enregistrés lorsque stdout est un terminal ; sinon, ils s'appliquent uniquement à l'invocation en cours. Par défaut, les modifications --open/--close/--show/--hide sont toujours enregistrées. La couleur est activée par défaut pour la sortie du terminal et désactivée pour les autres sorties. --temp ou --persist/--save remplace explicitement ces règles de sauvegarde.",
    .persistence_note_tty = "Message : {s} : enregistré car stdout est un terminal ; utilisez --temp pour l'appliquer uniquement à cet appel.",
    .persistence_note_non_tty = "Message : {s} : non enregistré car stdout n'est pas un terminal ; utilisez --persist/--save pour remplacer.",
    .persistence_note_semantic = "Message : {s} : enregistré car les modifications apportées à la vue du projet partagé sont enregistrées par défaut ; utilisez --temp pour les appliquer uniquement à cet appel.",
    .persistence_note_env = "Message : {s} : non enregistré car DIRTREE_TEMP=1 ; utilisez --persist/--save pour remplacer.",
    .persistence_note_mute = "Définissez DIRTREE_MUTE_PERSISTENCE_REASON=1 pour supprimer ce message d'information.",
    ...
    .help_opt_annotate = "  annotate PATH DESC Persister une note d'une ligne sur un fichier ou un répertoire (alias: note ; un DESC vide efface)",
