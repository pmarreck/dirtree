...
pub const strings = Strings{
    ...
    .help_opt_head = "  --head N           N줄 후 출력 중지",
    .help_opt_tail = "  --tail N           마지막 N줄만 표시 (tail -N로 파이프 권장)",
    .help_opt_only = "  --only PATH        하위 트리에 집중하고 형제 디렉터리를 접기 (반복 가능)",
    .warn_large_output_prefix = "경고: 출력이 약 ~",
    .warn_large_output_mid = "줄 (임계값: ",
    .warn_large_output_suffix = "). 고려하세요: --depth N, --head N 또는 --hide 패턴.",
    .warn_truncated_head_prefix = "(--head로 ",
    .warn_truncated_head_suffix = "줄 후 출력이 잘림)",
    .err_only_requires_path = "오류: --only에는 경로 인수가 필요합니다",
    .err_max_lines_requires_number = "오류: --max-lines에는 숫자 인수가 필요합니다",
    .err_head_requires_number = "오류: --head에는 숫자 인수가 필요합니다",
    .err_tail_requires_number = "오류: --tail에는 숫자 인수가 필요합니다",
