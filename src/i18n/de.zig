...
pub const strings = Strings{
    ...
    .help_opt_persist = "  --persist, --save  Speichern Sie auch diese Einstellungen (überschreibt non-TTY und DIRTREE_TEMP)",
    ...
    .help_behavior_text = "Präsentationseinstellungen werden gespeichert, wenn stdout ein Terminal ist; andernfalls gelten sie nur für den aktuellen Aufruf. Standardmäßig werden --open/--close/--show/--hide-Änderungen immer gespeichert. Die Farbe ist für die Terminalausgabe standardmäßig aktiviert und für andere Ausgaben deaktiviert. --temp oder --persist/--save überschreiben diese Speicherregeln explizit.",
    .persistence_note_tty = "Nachricht: {s}: gespeichert, da stdout ein Terminal ist; Verwenden Sie --temp, um es nur auf diesen Aufruf anzuwenden.",
    .persistence_note_non_tty = "Meldung: {s}: nicht gespeichert, da stdout kein Terminal ist; Verwenden Sie --persist/--save zum Überschreiben.",
    .persistence_note_semantic = "Meldung: {s}: gespeichert, da Änderungen an der freigegebenen Projektansicht standardmäßig gespeichert werden; Verwenden Sie --temp, um sie nur auf diesen Aufruf anzuwenden.",
    .persistence_note_env = "Meldung: {s}: nicht gespeichert, da DIRTREE_TEMP=1; Verwenden Sie --persist/--save zum Überschreiben.",
    .persistence_note_mute = "Legen Sie DIRTREE_MUTE_PERSISTENCE_REASON=1 fest, um diese Informationsmeldung zu unterdrücken.",
    ...
    .help_opt_annotate = "  annotate PATH DESC Eine einzeilige Notiz zu einer Datei oder einem Verzeichnis speichern (Alias: note; leeres DESC löscht)",
...
pub const aliases = LocaleAliases{
    .cli = &[_]CliAliasEntry{
        ...
        .{ .name = "--verknuepfungen", .arg = .hyperlinks },
