...
pub const strings = Strings{
    ...
    .help_opt_persist = "  --persist, --save  Salve essas configurações também (substitui non-TTY e DIRTREE_TEMP)",
    ...
    .help_behavior_text = "As configurações de apresentação são salvas quando stdout é um terminal; caso contrário, eles se aplicam apenas à invocação atual. Por padrão, as alterações de --open/--close/--show/--hide são sempre salvas. A cor está ativada por padrão para a saída do terminal e desativada para outras saídas. --temp ou --persist/--save substitui explicitamente essas regras de salvamento.",
    .persistence_note_tty = "Mensagem: {s}: salva porque stdout é um terminal; use --temp para aplicá-lo somente a esta chamada.",
    .persistence_note_non_tty = "Mensagem: {s}: não salva porque stdout não é um terminal; use --persist/--save para substituir.",
    .persistence_note_semantic = "Mensagem: {s}: salva porque as alterações na visualização do projeto compartilhada são salvas por padrão; use --temp para aplicá-los somente a esta chamada.",
    .persistence_note_env = "Mensagem: {s}: não salvo porque DIRTREE_TEMP=1; use --persist/--save para substituir.",
    .persistence_note_mute = "Configure DIRTREE_MUTE_PERSISTENCE_REASON=1 para suprimir esta mensagem informativa.",
    ...
    .help_opt_annotate = "  annotate PATH DESC Persistir uma nota de uma linha sobre um arquivo ou diretório (alias: note; DESC vazio limpa)",
