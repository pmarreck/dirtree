...
pub const strings = Strings{
    ...
    .help_opt_temp = "  -t, --temp         今回の実行のみ変更を適用（保存しない）",
    .help_opt_persist = "  --persist, --save  これらの設定も保存します (non-TTY および DIRTREE_TEMP をオーバーライドします)。",
    ...
    .help_opt_no_color = "  --no-color        ANSIカラーを有効/無効化（保存される）",
    ...
    .help_opt_no_hyperlinks = "  --no-hyperlinks   OSC8ハイパーリンクを有効/無効化（保存される）",
    ...
    .help_behavior_text = "プレゼンテーション設定は、stdout が端末の場合に保存されます。それ以外の場合は、現在の呼び出しにのみ適用されます。デフォルトでは、--open/--close/--show/--hide の変更は常に保存されます。デフォルトでは、端末出力では色がオンになり、他の出力ではオフになります。 --temp または --persist/--save は、これらの保存ルールを明示的にオーバーライドします。",
    .persistence_note_tty = "メッセージ: {s}: stdout は端末であるため保存されました。 --temp を使用して、この呼び出しにのみ適用します。",
    .persistence_note_non_tty = "メッセージ: {s}: stdout は端末ではないため保存されません。 --persist/--save を使用してオーバーライドします。",
    .persistence_note_semantic = "メッセージ: {s}: 共有プロジェクト ビューへの変更はデフォルトで保存されるため、保存されました。 --temp を使用して、この呼び出しにのみ適用します。",
    .persistence_note_env = "メッセージ: {s}: DIRTREE_TEMP=1 のため保存されませんでした。 --persist/--save を使用してオーバーライドします。",
    .persistence_note_mute = "この情報メッセージを抑制するには、DIRTREE_MUTE_PERSISTENCE_REASON=1 を設定します。",
    ...
    .err_path_requires_arg = "エラー: --path にはディレクトリ引数が必要です (en: Error: --path requires a directory argument)",
    ...
    .err_only_requires_path = "エラー: --only にはパス引数が必要です (en: Error: --only requires a path argument)",
    .err_max_lines_requires_number = "エラー: --max-lines には数値引数が必要です (en: Error: --max-lines requires a numeric argument)",
    .err_notes_requires_mode = "エラー: --notes には 'aligned' または 'inline' が必要です (en: Error: --notes requires 'aligned' or 'inline')",
    .err_sort_requires_mode = "エラー: --sort には 'modified' または 'alpha' が必要です (en: Error: --sort requires 'modified' or 'alpha')",
    .err_default_requires_value = "エラー: --default には少なくとも1つの値が必要です (en: Error: --default requires at least one value)",
    .err_default_state_conflict = "エラー: --default 状態の競合 (en: Error: --default state conflict)",
    .err_default_accepts = "エラー: --default は opened/closed を受け付けます (en: Error: --default accepts opened/closed)",
    .err_open_requires_dir = "エラー: --open には少なくとも1つのディレクトリが必要です (en: Error: --open requires at least one directory)",
    .err_close_requires_dir = "エラー: --close には少なくとも1つのディレクトリが必要です (en: Error: --close requires at least one directory)",
    .err_show_requires_path = "エラー: --show には少なくとも1つのパスが必要です (en: Error: --show requires at least one path)",
    .err_hide_requires_path = "エラー: --hide には少なくとも1つのパスが必要です (en: Error: --hide requires at least one path)",
    .err_unknown_option = "不明なオプション (en: Unknown option)",
    .err_not_a_directory = "エラー: '{s}' はディレクトリではありません (en: Error: '{s}' is not a directory)",
    .err_regex_empty = "エラー: 正規表現パターンは空にできません (en: Error: regex pattern must not be empty)",
    .err_paths_must_be_relative = "エラー: {s} のパスは相対パスでなければなりません (先頭に '/' 不可): {s} (en: Error: {s} paths must be relative (no leading '/'): {s})",
    .err_out_of_memory = "メモリ不足 (en: Out of memory)",
    .err_regex_conflict_path = "エラー: パス '{s}' がopenパターンとcloseパターンの両方に一致します (en: Error: path '{s}' matches both open and close patterns)",
    .err_regex_conflict_open = "  openパターン: {s} (en:   open pattern: {s})",
    .err_regex_conflict_close = "  closeパターン: {s} (en:   close pattern: {s})",
    .err_regex_invalid = "エラー: 無効な正規表現パターン: {s} (en: Error: invalid regex pattern: {s})",
    .err_unknown_lang = "エラー: 不明な言語コード '{s}'。利用可能: {s} (en: Error: unknown language code '{s}'. Available: {s})",
    ...
    .help_opt_annotate = "  annotate PATH DESC ファイルまたはディレクトリに関する1行の注釈を保存 (alias: note; 空の DESC で消去)",
    ...
    // ── その他 ──────────────────────────────────────────────────
    .err_test_bin_run = "エラー: DIRTREE_TEST_BIN を実行できませんでした: {s} (en: Error: could not run DIRTREE_TEST_BIN: {s})",
    .err_test_bin_wait = "エラー: DIRTREE_TEST_BIN の待機に失敗しました (en: Error: could not wait for DIRTREE_TEST_BIN)",
    .err_render_tree = "ツリーの描画エラー: {} (en: Error rendering tree: {})",
};
