...
pub const strings = Strings{
    ...
    .warn_negation_advice = "特定のパスに絞るには --only PATH を、肯定的なフィルタには --show /PAT/ を使ってください（show ルールは hide より優先されます）。これらのルールは .dirtree-state に保存されます。複雑になったり重なったりしたら手で編集できるプレーンテキストです。",
