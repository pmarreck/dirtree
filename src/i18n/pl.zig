...
pub const strings = Strings{
    ...
    .help_opt_temp = "  -t, --temp         Zastosuj zmiany tylko dla tego uruchomienia (bez zapisu)",
    .help_opt_persist = "  --persist, --save  Zapisz także te ustawienia (zastępuje non-TTY i DIRTREE_TEMP)",
    ...
    .help_opt_no_color = "  --no-color        Włącz/wyłącz kolory ANSI (zapisywane)",
    ...
    .help_opt_no_hyperlinks = "  --no-hyperlinks   Włącz/wyłącz hiperłącza OSC8 (zapisywane)",
    ...
    .help_behavior_text = "Ustawienia prezentacji są zapisywane, gdy stdout jest terminalem; w przeciwnym razie mają zastosowanie tylko do bieżącego wywołania. Domyślnie zmiany --open/--close/--show/--hide są zawsze zapisywane. Kolor jest domyślnie włączony dla wyjścia terminala i wyłączony dla innego wyjścia. --temp lub --persist/--save jawnie zastępuje te reguły zapisywania.",
    .persistence_note_tty = "Komunikat: {s}: zapisany, ponieważ stdout jest terminalem; użyj --temp, aby zastosować go tylko do tego wywołania.",
    .persistence_note_non_tty = "Komunikat: {s}: nie został zapisany, ponieważ stdout nie jest terminalem; użyj --persist/--save, aby zastąpić.",
    .persistence_note_semantic = "Komunikat: {s}: zapisany, ponieważ zmiany w udostępnionym widoku projektu są domyślnie zapisywane; użyj --temp, aby zastosować je tylko do tego wywołania.",
    .persistence_note_env = "Wiadomość: {s}: nie zapisano, ponieważ DIRTREE_TEMP=1; użyj --persist/--save, aby zastąpić.",
    .persistence_note_mute = "Ustaw DIRTREE_MUTE_PERSISTENCE_REASON=1, aby ukryć ten komunikat informacyjny.",
    ...
    .help_opt_annotate = "  annotate PATH DESC Zapisz notatkę o pliku lub katalogu (alias: note; pusty DESC czyści)",
