...
pub const strings = Strings{
    ...
    .warn_negation_advice = "Щоб зосередитися на шляху, використовуйте --only PATH; для позитивного фільтра — --show /PAT/ (правила show мають пріоритет над hide). Ці правила зберігаються у .dirtree-state — текстовому файлі, який можна редагувати вручну, коли вони ускладнюються чи перетинаються.",
