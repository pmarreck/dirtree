...
pub const strings = Strings{
    ...
    .help_opt_persist = "  --persist, --save  Збережіть також ці параметри (перевизначає non-TTY і DIRTREE_TEMP)",
    ...
    .help_behavior_text = "Параметри презентації зберігаються, якщо stdout є терміналом; інакше вони застосовуються лише до поточного виклику. За замовчуванням зміни --open/--close/--show/--hide завжди зберігаються. Колір за замовчуванням увімкнено для виводу терміналу та вимкнено для іншого виводу. --temp або --persist/--save явно замінює ці правила збереження.",
    .persistence_note_tty = "Повідомлення: {s}: збережено, оскільки stdout є терміналом; використовуйте --temp, щоб застосувати його лише до цього виклику.",
    .persistence_note_non_tty = "Повідомлення: {s}: не збережено, оскільки stdout не є терміналом; використовуйте --persist/--save для перевизначення.",
    .persistence_note_semantic = "Повідомлення: {s}: збережено, оскільки зміни в спільному поданні проекту зберігаються за замовчуванням; використовуйте --temp, щоб застосувати їх лише до цього виклику.",
    .persistence_note_env = "Повідомлення: {s}: не збережено, оскільки DIRTREE_TEMP=1; використовуйте --persist/--save для перевизначення.",
    .persistence_note_mute = "Налаштуйте DIRTREE_MUTE_PERSISTENCE_REASON=1 для придушення цього інформаційного повідомлення.",
    ...
    .help_opt_annotate = "  annotate PATH DESC Зберегти однорядкову нотатку про файл або каталог (аліас: note; порожній DESC очищає)",
