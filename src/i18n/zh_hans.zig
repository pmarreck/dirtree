...
pub const strings = Strings{
    ...
    .help_opt_head = "  --head N           在 N 行后停止输出",
    .help_opt_tail = "  --tail N           仅显示最后 N 行 (建议改用管道 tail -N)",
    .help_opt_only = "  --only PATH        聚焦某个子树，折叠同级目录 (可重复)",
    .warn_large_output_prefix = "警告: 输出约 ~",
    .warn_large_output_mid = " 行 (阈值: ",
    .warn_large_output_suffix = ")。请考虑: --depth N、--head N 或 --hide 模式。",
    .warn_truncated_head_prefix = "(输出在 ",
    .warn_truncated_head_suffix = " 行后被 --head 截断)",
    ...
    .err_head_requires_number = "错误: --head 需要一个数字参数 (en: Error: --head requires a numeric argument)",
    .err_tail_requires_number = "错误: --tail 需要一个数字参数 (en: Error: --tail requires a numeric argument)",
