...
pub const strings = Strings{
    ...
    .help_opt_persist = "  --persist, --save  Sačuvajte i ove postavke (zamjenjuje non-TTY i DIRTREE_TEMP)",
    ...
    .help_behavior_text = "Postavke prezentacije se pohranjuju kada je stdout terminal; inače se primjenjuju samo na trenutno pozivanje. Po defaultu, --open/--close/--show/--hide promjene se uvijek spremaju. Boja je podrazumevano uključena za terminalski izlaz i isključena za drugi izlaz. --temp ili --persist/--save eksplicitno nadjačava ova pravila spremanja.",
    .persistence_note_tty = "Poruka: {s}: sačuvano jer je stdout terminal; koristite --temp da ga primijenite samo na ovu invokaciju.",
    .persistence_note_non_tty = "Poruka: {s}: nije spremljeno jer stdout nije terminal; koristite --persist/--save za nadjačavanje.",
    .persistence_note_semantic = "Poruka: {s}: sačuvano jer su promjene u dijeljenom prikazu projekta pohranjene po defaultu; koristite --temp da ih primenite samo na ovu invokaciju.",
    .persistence_note_env = "Poruka: {s}: nije sačuvano jer DIRTREE_TEMP=1; koristite --persist/--save za nadjačavanje.",
