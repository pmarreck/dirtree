...
pub const strings = Strings{
    ...
    .warn_negation_advice = "برای تمرکز روی یک مسیر از --only PATH و برای فیلتر مثبت از --show /PAT/ استفاده کنید (قوانین show بر hide اولویت دارند). این قوانین در .dirtree-state ذخیره می‌شوند؛ یک فایل متنی که هنگام پیچیده یا هم‌پوشان شدن می‌توانید دستی ویرایش کنید.",
