...
pub const strings = Strings{
    ...
    .help_opt_persist = "  --persist, --save  Salvați și aceste setări (înlocuiește non-TTY și DIRTREE_TEMP)",
    ...
    .help_behavior_text = "Setările de prezentare sunt salvate atunci când stdout este un terminal; în caz contrar ele se aplică numai invocarii curente. În mod implicit, modificările --open/--close/--show/--hide sunt întotdeauna salvate. Culoarea este activată în mod implicit pentru ieșirea terminalului și dezactivată pentru alte ieșiri. --temp sau --persist/--save anulează în mod explicit aceste reguli de salvare.",
    .persistence_note_tty = "Mesaj: {s}: salvat deoarece stdout este un terminal; utilizați --temp pentru a-l aplica numai la această invocare.",
    .persistence_note_non_tty = "Mesaj: {s}: nu a fost salvat deoarece stdout nu este un terminal; utilizați --persist/--save pentru a înlocui.",
    .persistence_note_semantic = "Mesaj: {s}: salvat deoarece modificările la vizualizarea partajată a proiectului sunt salvate implicit; utilizați --temp pentru a le aplica numai la această invocare.",
    .persistence_note_env = "Mesaj: {s}: nu a fost salvat deoarece DIRTREE_TEMP=1; utilizați --persist/--save pentru a înlocui.",
    .persistence_note_mute = "Setați DIRTREE_MUTE_PERSISTENCE_REASON=1 pentru a suprima acest mesaj informațional.",
    ...
    .help_opt_annotate = "  annotate PATH DESC Salvează o notă de o linie pentru un fișier sau director (alias: note; DESC gol o șterge)",
