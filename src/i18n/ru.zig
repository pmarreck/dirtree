...
pub const strings = Strings{
    ...
    .warn_negation_advice = "Чтобы сфокусироваться на пути, используйте --only PATH; для положительного фильтра — --show /PAT/ (правила show важнее hide). Эти правила сохраняются в .dirtree-state — текстовый файл, который можно править вручную, когда они усложняются или пересекаются.",
