...
pub const strings = Strings{
    ...
    .help_opt_temp = "  -t, --temp         Применить изменения только для этого запуска (не сохраняется)",
    .help_opt_persist = "  --persist, --save  Сохраните и эти настройки (переопределяет non-TTY и DIRTREE_TEMP)",
    ...
    .help_opt_no_color = "  --no-color        Включить/выключить цвета ANSI (сохраняется)",
    ...
    .help_opt_no_hyperlinks = "  --no-hyperlinks   Включить/выключить гиперссылки OSC8 (сохраняется)",
    ...
    .help_behavior_text = "Настройки презентации сохраняются, если stdout является терминалом; в противном случае они применяются только к текущему вызову. По умолчанию изменения --open/--close/--show/--hide всегда сохраняются. По умолчанию цвет включен для вывода через терминал и выключен для другого вывода. --temp или --persist/--save явно отменяют эти правила сохранения.",
    .persistence_note_tty = "Сообщение: {s}: сохранено, поскольку stdout — это терминал; используйте --temp, чтобы применить его только к этому вызову.",
    .persistence_note_non_tty = "Сообщение: {s}: не сохранено, поскольку stdout не является терминалом; используйте --persist/--save для переопределения.",
    .persistence_note_semantic = "Сообщение: {s}: сохранено, поскольку изменения в общем представлении проекта сохраняются по умолчанию; используйте --temp, чтобы применить их только к этому вызову.",
    .persistence_note_env = "Сообщение: {s}: не сохранено, потому что DIRTREE_TEMP=1; используйте --persist/--save для переопределения.",
    .persistence_note_mute = "Установите DIRTREE_MUTE_PERSISTENCE_REASON=1, чтобы подавить это информационное сообщение.",
    ...
    .help_opt_annotate = "  annotate PATH DESC Сохранить однострочную заметку о файле или каталоге (алиас: note; пустой DESC очищает)",
