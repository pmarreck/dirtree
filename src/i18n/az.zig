...
pub const strings = Strings{
    ...
    .help_opt_temp = "  -t, --temp         Dəyişiklikləri yalnız bu işləmə üçün tətbiq et (saxlanılmır)",
    .help_opt_persist = "  --persist, --save  Bu parametrləri də yadda saxlayın (non-TTY və DIRTREE_TEMP-nu ləğv edir)",
    ...
    .help_opt_no_color = "  --no-color        ANSI rənglərini aç/bağla (saxlanılır)",
    .help_opt_no_orphan_warning = "  --no-orphan-warning Kimsəsiz qeyd xəbərdarlığını gizlət",
    .help_opt_notes = "  --no-notes/--show-notes Sətiriçi qeydləri gizlət/göstər (DIRTREE_HIDE_NOTES=1 standart gizlədir)",
    .help_opt_notes_mode = "  --notes MODE       Qeyd düzümü: aligned (standart) və ya inline",
    .help_opt_notes_leader = "  --notes-leader     Adlardan düzülmüş qeydlərə nöqtəli bələdçi xətləri çək",
    .warn_orphaned_prefix = "Qeyd: ",
    .warn_orphaned_suffix = " annotasiya mövcud olmayan yollara işarə edir. Baxmaq üçün 'orphaned-notes', silmək üçün 'purge-orphaned-notes' işlədin.",
    .warn_negation_intro = "qeyd: inkar edilmiş regex niyyəti tərsinə çevirə bilər — !/PAT/ ƏKSİNƏ uyğun gəlir, baştakı (?!...) də inkardır; ikisini birləşdirmək ikiqat inkar yaradır:",
    .warn_negation_advice = "Bir yola fokuslanmaq üçün {s} PATH; müsbət filtr üçün {s} /PAT/ istifadə edin (show qaydaları hide-dan üstündür). Bu qaydalar .dirtree-state-də saxlanılır — mürəkkəbləşəndə və ya üst-üstə düşəndə əl ilə redaktə edilə bilən mətn faylı.",
    .help_opt_no_hyperlinks = "  --no-hyperlinks   OSC8 bağlantılarını aç/bağla (saxlanılır)",
    ...
    .help_opt_hide = "  --hide YOLA...     Nisbi yollar\xc4\xb1 gizl\xc9\x99; regex /\xc5\x9fablon/ v\xc9\x99 ya !/\xc5\x9fablon/ (t\xc9\x99krarlanabilir) (yalnız bəzi yollar? --only istifadə edin)",
    ...
    .help_behavior_text = "stdout terminal olduqda təqdimat parametrləri yadda saxlanılır; əks halda onlar yalnız cari çağırışa şamil edilir. Varsayılan olaraq, --open/--close/--show/--hide dəyişiklikləri həmişə yadda saxlanılır. Rəng defolt olaraq terminal çıxışı üçün aktivdir, digər çıxış üçün isə söndürülür. --temp və ya --persist/--save açıq şəkildə bu saxlama qaydalarını ləğv edir.",
    .persistence_note_tty = "Mesaj: {s}: saxlandı, çünki stdout terminaldır; onu yalnız bu çağırışa tətbiq etmək üçün --temp istifadə edin.",
    .persistence_note_non_tty = "Mesaj: {s}: yadda saxlanılmadı, çünki stdout terminal deyil; ləğv etmək üçün --persist/--save istifadə edin.",
    .persistence_note_semantic = "Mesaj: {s}: paylaşılan layihə görünüşünə edilən dəyişikliklər defolt olaraq yadda saxlandığı üçün saxlanıldı; onları yalnız bu çağırışa tətbiq etmək üçün --temp istifadə edin.",
    .persistence_note_env = "Mesaj: {s}: yadda saxlanılmadı, çünki DIRTREE_TEMP=1; ləğv etmək üçün --persist/--save istifadə edin.",
    .persistence_note_mute = "Bu məlumat mesajını yatırtmaq üçün DIRTREE_MUTE_PERSISTENCE_REASON=1 seçin.",
    ...
    .err_path_requires_arg = "Xəta: --path qovluq arqumenti tələb edir (en: Error: --path requires a directory argument)",
    ...
    .err_only_requires_path = "Xəta: --only yol arqumenti tələb edir (en: Error: --only requires a path argument)",
    .err_max_lines_requires_number = "X\\xc9\\x99ta: --max-lines r\\xc9\\x99q\\xc9\\x99mli arqument t\\xc9\\x99l\\xc9\\x99b edir (en: Error: --max-lines requires a numeric argument)",
    .err_notes_requires_mode = "Xəta: --notes 'aligned' və ya 'inline' tələb edir (en: Error: --notes requires 'aligned' or 'inline')",
    .err_sort_requires_mode = "X\xc9\x99ta: --sort 'modified' v\xc9\x99 ya 'alpha' t\xc9\x99l\xc9\x99b edir (en: Error: --sort requires 'modified' or 'alpha')",
    .err_default_requires_value = "X\xc9\x99ta: --default \xc9\x99n az\xc4\xb1 bir d\xc9\x99y\xc9\x99r t\xc9\x99l\xc9\x99b edir (en: Error: --default requires at least one value)",
    .err_default_state_conflict = "X\xc9\x99ta: --default v\xc9\x99ziyy\xc9\x99t ziddiyy\xc9\x99ti (en: Error: --default state conflict)",
    .err_default_accepts = "X\xc9\x99ta: --default opened/closed q\xc9\x99bul edir (en: Error: --default accepts opened/closed)",
    .err_open_requires_dir = "X\xc9\x99ta: --open \xc9\x99n az\xc4\xb1 bir qovluq t\xc9\x99l\xc9\x99b edir (en: Error: --open requires at least one directory)",
    .err_close_requires_dir = "X\xc9\x99ta: --close \xc9\x99n az\xc4\xb1 bir qovluq t\xc9\x99l\xc9\x99b edir (en: Error: --close requires at least one directory)",
    .err_show_requires_path = "X\xc9\x99ta: --show \xc9\x99n az\xc4\xb1 bir yol t\xc9\x99l\xc9\x99b edir (en: Error: --show requires at least one path)",
    .err_hide_requires_path = "X\xc9\x99ta: --hide \xc9\x99n az\xc4\xb1 bir yol t\xc9\x99l\xc9\x99b edir (en: Error: --hide requires at least one path)",
    .err_unknown_option = "Nabilinm\xc9\x99y\xc9\x99n se\xc3\xa7im (en: Unknown option)",
    .err_not_a_directory = "X\xc9\x99ta: '{s}' qovluq deyil (en: Error: '{s}' is not a directory)",
    .err_regex_empty = "X\xc9\x99ta: regex \xc5\x9fablonu bo\xc5\x9f ola bilm\xc9\x99z (en: Error: regex pattern must not be empty)",
    .err_paths_must_be_relative = "X\xc9\x99ta: {s} yollar\xc4\xb1 nisbi olmal\xc4\xb1d\xc4\xb1r (\xc9\x99vv\xc9\x99lind\xc9\x99 '/' olmadan): {s} (en: Error: {s} paths must be relative (no leading '/'): {s})",
    .err_out_of_memory = "Yaddaş çatışmazlığı (en: Out of memory)",
    .err_regex_conflict_path = "X\xc9\x99ta: '{s}' yolu h\xc9\x99m a\xc3\xa7\xc4\xb1q, h\xc9\x99m ba\xc4\x9fl\xc4\xb1 \xc5\x9fablonlara uy\xc4\x9fundur (en: Error: path '{s}' matches both open and close patterns)",
    .err_regex_conflict_open = "  a\xc3\xa7\xc4\xb1q \xc5\x9fablonu: {s} (en:   open pattern: {s})",
    .err_regex_conflict_close = "  ba\xc4\x9fl\xc4\xb1 \xc5\x9fablonu: {s} (en:   close pattern: {s})",
    .err_regex_invalid = "Xəta: yanlış regex şablonu: {s} (en: Error: invalid regex pattern: {s})",
    .err_unknown_lang = "X\xc9\x99ta: nabilinm\xc9\x99y\xc9\x99n dil kodu '{s}'. M\xc3\xb6vcud: {s} (en: Error: unknown language code '{s}'. Available: {s})",
    ...
    .help_opt_annotate = "  annotate PATH DESC Fayl və ya qovluq haqqında birsətrəli qeyd əlavə et (alias: note; boş DESC qeydi silir)",
    ...
    .purge_none = "Silinəcək kimsəsiz qeyd yoxdur.",
    ...
    // ── Dig\xc9\x99r ──────────────────────────────────────────────────────
    .err_test_bin_run = "X\xc9\x99ta: DIRTREE_TEST_BIN i\xc5\x9f\xc9\x99 sal\xc4\xb1na bilm\xc9\x99di: {s} (en: Error: could not run DIRTREE_TEST_BIN: {s})",
    .err_test_bin_wait = "X\xc9\x99ta: DIRTREE_TEST_BIN g\xc3\xb6zl\xc9\x99nil\xc9\x99 bilm\xc9\x99di (en: Error: could not wait for DIRTREE_TEST_BIN)",
    .err_render_tree = "A\xc4\x9fac\xc4\xb1n g\xc3\xb6st\xc9\x99rilm\xc9\x99sind\xc9\x99 x\xc9\x99ta: {} (en: Error rendering tree: {})",
};
...
pub const aliases = LocaleAliases{
    .cli = &[_]CliAliasEntry{
        ...
        .{ .name = "--reng", .arg = .color },
        .{ .name = "--kimsesiz-xeberdarliq-yox", .arg = .no_orphan_warning },
        .{ .name = "--qeydsiz", .arg = .no_notes },
        .{ .name = "--qeydleri-goster", .arg = .show_notes },
        .{ .name = "--qeyd-duzumu", .arg = .notes },
        .{ .name = "--noqteli-belledci", .arg = .note_leader },
        .{ .name = "--linkler-yok", .arg = .no_hyperlinks },
        .{ .name = "--baglantilar", .arg = .hyperlinks },
        ...
        .{ .name = "--sinaq", .arg = .@"test" },
        .{ .name = "--dil", .arg = .lang },
        .{ .name = "--muveqqeti", .arg = .temporary },
        .{ .name = "qeyd", .arg = .annotate },
        .{ .name = "kimsesiz-qeydler", .arg = .orphaned_notes },
        .{ .name = "kimsesiz-qeydleri-temizle", .arg = .purge_orphaned_notes },
    },
    .env = &[_]EnvAliasEntry{
