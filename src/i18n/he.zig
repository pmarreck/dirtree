...
pub const strings = Strings{
    ...
    .help_opt_persist = "  --persist, --save  שמור גם את ההגדרות הללו (עובר את non-TTY ו- DIRTREE_TEMP)",
    ...
    .help_behavior_text = "הגדרות המצגת נשמרות כאשר stdout הוא מסוף; אחרת הם חלים רק על הפניה הנוכחית. כברירת מחדל, שינויים --open/--close/--show/--hide נשמרים תמיד. צבע מופעל כברירת מחדל עבור פלט מסוף וכבוי עבור פלט אחר. --temp או --persist/--save עוקפים במפורש כללי שמירה אלה.",
    .persistence_note_tty = "הודעה: {s}: נשמרה כי stdout הוא מסוף; השתמש ב---temp כדי להחיל אותו רק על קריאה זו.",
    .persistence_note_non_tty = "הודעה: {s}: לא נשמרה כי stdout אינו מסוף; השתמש ב---persist/--save כדי לעקוף.",
    .persistence_note_semantic = "הודעה: {s}: נשמר מכיוון ששינויים בתצוגת הפרויקט המשותף נשמרים כברירת מחדל; השתמש ב---temp כדי להחיל אותם רק על קריאה זו.",
    .persistence_note_env = "הודעה: {s}: לא נשמרה בגלל DIRTREE_TEMP=1; השתמש ב---persist/--save כדי לעקוף.",
    .persistence_note_mute = "הגדר את DIRTREE_MUTE_PERSISTENCE_REASON=1 כדי לדכא הודעת מידע זו.",
    ...
    .help_opt_annotate = "  annotate PATH DESC שמירת הערה בשורה אחת על קובץ או תיקייה (alias: note; DESC ריק מוחק את ההערה)",
