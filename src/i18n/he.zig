...
pub const strings = Strings{
    ...
    .warn_negation_advice = "כדי להתמקד בנתיב אחד השתמש ב--only PATH; לסינון חיובי ב--show /PAT/ (כללי show גוברים על hide). הכללים נשמרים ב-.dirtree-state, קובץ טקסט שניתן לערוך ביד כשהם נעשים מורכבים או חופפים.",
