...
pub const strings = Strings{
    ...
    .help_opt_persist = "  --persist, --save  Mentse el ezeket a beállításokat is (felülbírálja a non-TTY és DIRTREE_TEMP)",
    ...
    .help_opt_no_color = "  --no-color        ANSI-színek be-/kikapcsolása (mentve)",
    ...
    .help_opt_no_hyperlinks = "  --no-hyperlinks   OSC8-hivatkozások be-/kikapcsolása (mentve)",
    ...
    .help_behavior_text = "A prezentációs beállítások mentésre kerülnek, ha a stdout terminál; egyébként csak az aktuális invokációra vonatkoznak. Alapértelmezés szerint a --open/--close/--show/--hide módosítások mindig mentésre kerülnek. A szín alapértelmezés szerint be van kapcsolva a terminálkimenetnél, és ki van kapcsolva a többi kimenetnél. A --temp vagy a --persist/--save kifejezetten felülírja ezeket a mentési szabályokat.",
    .persistence_note_tty = "Üzenet: {s}: mentve, mert a stdout terminál; használja a --temp-t, hogy csak erre a meghívásra alkalmazza.",
    .persistence_note_non_tty = "Üzenet: {s}: nincs mentve, mert a stdout nem terminál; a felülbíráláshoz használja a --persist/--save-t.",
    .persistence_note_semantic = "Üzenet: {s}: mentve, mert a megosztott projektnézet módosításai alapértelmezés szerint mentésre kerülnek; használja a --temp-t, hogy csak erre a meghívásra alkalmazza őket.",
    .persistence_note_env = "Üzenet: {s}: nincs mentve, mert DIRTREE_TEMP=1; a felülbíráláshoz használja a --persist/--save-t.",
    .persistence_note_mute = "Állítsa be a DIRTREE_MUTE_PERSISTENCE_REASON=1-t az információs üzenet letiltására.",
    ...
    .help_opt_annotate = "  annotate PATH DESC Mentse meg egy egysoros megjegyzést egy fájlról vagy mappáról (alias: note; üresen hagyott DESC törli)",
...
pub const aliases = LocaleAliases{
    .cli = &[_]CliAliasEntry{
        ...
        .{ .name = "--hivatkozasok", .arg = .hyperlinks },
